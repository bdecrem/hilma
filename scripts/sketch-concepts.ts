...
// (as Amber) to sketch each keyword as a 26×10 silhouette, and writes
...
function parseAsciiGrid(ascii: string): number[][] {
  ...
  const gridLines = lines.filter(l => /^[.#\s]+$/.test(l))
...
You sketch in a strict 26 columns × 10 rows grid, using '#' for filled cells and '.' for empty cells. Each row is exactly 26 characters. You have 10 rows. You are making minimal, recognizable silhouettes — ideograms, not illustrations. Think bathroom-sign clarity, not detail. A compass, a bird, a candle — that's the register. Your sketches should feel like the mood.`

  const userPrompt = `Sketch each of these keywords as a 26×10 silhouette. One sketch per keyword.
...
- Each sketch is EXACTLY 10 rows of EXACTLY 26 characters
