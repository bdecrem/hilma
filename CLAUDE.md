...
- **Phone numbers** (2026-09-19): `normalizePhone` in `core.ts` reads what people type, not just E.164 — the iPhone phone keypad hides "+" behind the +*# key, so a number outside the US arrives as `44 7911 123456`, `0044 …` or `+44 (0)7911 …`; all become `+447911123456`. A national number with its trunk zero (`07911 123456`) is refused with a message asking for the country code, and the code step shows the number as it was read. Bart's sister (+44) could not sign in before this: every attempt was a 400 with a small "Enter a phone number." and no code row was ever written. Checks: `npx tsx scripts/onething/phone-check.ts` (pure) and `node scripts/onething/pw-signin.mjs <shots-dir>` (the form in mobile WebKit; fictional numbers only).
