...
Landing mode (2026-09-06): a signed-out visitor to `/jam` sees a landing (built into `AuthScreen.tsx`), not a bare login form — hero (72px monogram mark + wordmark + one-line tagline + one sentence on what it does), two stacked full-width keys ("Start a track" / "Sign in"), the public `Catalog`, then the sign-in/signup form (closed by default for a first-time visitor, opened by a key tap; auto-opens on repeat visits via the `jam:seen` localStorage flag, and whenever a `?remix=` hint is present). Logo sources: `misc/jambotlogos/` (`jambot-monogram-{light,dark}.png` 1024², `jambot-icons.zip` with matching SVGs) — the monogram (ink tile / putty "J" / orange dot) is the app icon and site mark, the grid stays a UI-only motif. Exported assets live in `public/jam/`: `mark-dark.png`/`.svg` (256², dark tile, used in the landing hero and inlined as base64 in `mark-dark-b64.ts` for the edge-runtime `opengraph-image.tsx`) and the site's `favicon.ico`/`icon-192.png`/`icon-512.png`/`apple-touch-icon.png` (from the light monogram, wired via `layout.tsx`'s `metadata.icons`).
