...
| **Token Surfers (iOS + Mac)** | `apps/tokensurfers/` + `src/app/api/surf/llm/` + `src/lib/surf/` | Xcode (XcodeGen; Catalyst for Mac) | Brainrot vibe-coding app from `misc/3.MP4` (2026-09-23): Splat, an Opus 5.5 agent looping on the phone, builds one-file HTML apps while you play Token Surfers underneath — a real runner since 2026-09-24 (chunk-authored track, ride-able ramp trains, roof coins, game over, chiptune loop, the surfer = Splat with legs, scarf, antenna and a `>_` backpack) with a **global leaderboard** (`surf_scores`, `/api/surf/scores`) and solo play from Home. Tokens become coins, tool calls become terminal trains, bugs crawl onto the track. The server route owns the prompt and tools and streams SSE through. See `apps/tokensurfers/CLAUDE.md` |
