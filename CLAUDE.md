...
A per-device setting, **not the default**: Profile → Voice → **Voice engine: GPT-Live / ElevenLabs + Claude** (`VoiceEngine` in `apps/feynd/Feynd/DodoVoiceClient.swift`, UserDefaults `voiceEngine`). Same five voice surfaces, same screens, hold-to-talk, finish route and graders. Reference: [`docs/f2-eleven-voice-reference.md`](docs/f2-eleven-voice-reference.md).
