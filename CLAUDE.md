...
- **Doodles** (2026-09-23): every entry gets a margin doodle drawn by Opus 5.5 at low effort (`src/lib/onething/doodle.ts`, schema 005 applied: `doodle` = sanitized SVG element markup for a 340×170 viewBox, `doodle_alt`, `doodle_word`, `doodled_at`). `recordEntry` / `editEntryLine` call `scheduleDoodle` (next's `after()`; outside a request it draws inline), so a new sentence, an added thought or an edit redraws the day; `/api/onething/me` schedules up to four missing ones per visit. Structured output (`output_config.format`), the three example doodles from Bart's September page in the system prompt, the last three days' sentences + alts so it doesn't repeat itself. Words: `wordAllowed()` opens only when nothing worded sits in the previous three days, and the day's instruction differs (none at all / a word or two is welcome), so a worded doodle lands every four days at most — in a redo of eight days it used two. The sanitizer keeps path/circle/ellipse/rect/line/polyline/polygon/g (+ `<text>` on allowed days) and the classes `r`/`thin`/`f`/`fr`; the page styles them (`.ot-doodle` in onething.css). Checks: `npx tsx scripts/onething/doodle-check.ts` (sanitizer + gate), `scripts/onething/doodle-backfill.ts [--redo] [--user id]` (real calls, ~5 s each; the 40 entries of 2026-09-23 were backfilled), `node scripts/onething/pw-doodle-shot.mjs <png>` against a dev server on 3218 with `ONETHING_DEV_AS` set. One draw of 41 failed transiently ("nothing drawable came back") and passed on retry; the error now logs what came back. **Margins experiment** (same day, unlisted, `/onething/alt`, `src/app/onething/alt/`): a firm two-column sheet. Rules run the full page width; the words keep to the writing column; the margin column on the right holds every doodle in one slot (232×140 on desktop, 132×112 on phones — whole rules tall), top-aligned with the entry's first line, centred, ±2° by day parity and nothing else random. Two things make them read as one hand: `Fig` crops each doodle to its ink after mount (`getBBox` → viewBox, never scaled past `PEN_SCALE` 0.8 px per canvas unit, so small drawings stay small) and strokes are `vector-effect: non-scaling-stroke` (one pen at every scale). A row is 6 + content + 22; a drawn row is the slot plus one rule; empty days fold into one two-rule gap labelled with the range. Read-only (no composer, no garden). Takes one (doodles floated inside the text column) and two (loose in the margin with random shove, ink over words) were rejected as messy on 2026-09-23. `node scripts/onething/pw-alt-shot.mjs <png>` (phone) and `pw-alt-desk.mjs <png>` (1280 wide) shoot it.
