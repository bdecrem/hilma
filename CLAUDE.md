...
Two engines on ElevenLabs because an engine has one `ws_url`: "Dodo (dev)" → bridge `/ws/dev` → `localhost:3100`; "Dodo" → `/ws/prod` → `feynd.cc`. `ELEVEN_SPEECH_ENGINE_ID` is the dev one in `.env.local` and the prod one on Vercel; `DODO_BRIDGE_SECRET` and `ELEVENLABS_API_KEY` must be on Vercel too. The bridge runs on the Mac mini since 2026-09-21 (it spent its first day on Bart's MacBook Air, whose leftover job must be uninstalled — see the bridge README, which also has how to ship a bridge change to the mini). If Dodo says "Voice failed" on this engine, check `tail ~/Library/Logs/dodo-voice-bridge.log` on the host first. tunn3l's HTTP mode does not pass WebSocket upgrades, hence Cloudflare.
