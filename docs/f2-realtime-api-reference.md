...
The env voice is only the *default*. Per-user overrides live on `f2_users` (`realtime_voice`, `voice_style`, migration 026) and are managed via `GET/PUT /api/f2/voice-prefs`; both `/api/f2/realtime/session` and `/api/f4/walk/session` apply them at mint time (`getVoicePrefs` + `applyVoiceStyle` in `src/lib/f2/realtime.ts`). The pickable catalog is `REALTIME_VOICES` there; Dodo bundles a preview clip per voice (`apps/feynd/Feynd/VoicePreviews/`, regenerated by `scripts/generate-voice-previews.mjs` — rerun it whenever the catalog changes).
