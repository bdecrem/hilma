...
//   POST /api/surf/apps { client_id, title, emoji, prompt, html, remix_of? } → { app }   (signed in)
...
export async function POST(req: NextRequest) {
  ...
  if (html.trim().length < 20) return err('there is no app to publish yet', 400)
  ...
  try {
    const app = await publish({
      ...
      prompt: cleanPrompt(body.prompt), html,
