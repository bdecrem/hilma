...
export const EXPLANATIONS: Record<number, string> = {
  ...
  61: "From L60's 2D fragment shader to actual 3D — a ray-marched volumetric noise cloud that you can see *into*. Each pixel marches a ray through a 3D fbm volume, sampling density at every step, and at lit samples shoots a second ray toward the cursor for a cheap soft-shadow. Front-to-back alpha accumulation builds the cloud's depth and self-occlusion. Drag horizontally to orbit the volume; tap to bloom the light through it. The volume drifts on its z-axis over time so the structure is never quite static.",
  62: "From L61's volumetric noise (no surface) to actual 3D geometry with an explicit smooth surface. Five spheres orbit a center at different frequencies and radii; the SDF combines them via cubic-polynomial smooth-min so they merge into a single morphing blob — like a drop of mercury that won't settle on a shape. Per pixel: ray-march to the surface, estimate the normal via 6-tap finite differences, compute Lambert diffuse from a cursor-driven light, march a soft-shadow ray, and add a Fresnel rim that shifts the cream surface toward lime at grazing angles. Drag horizontally to orbit; tap to shudder the spheres briefly (perturbs each radius by 45% before settling back).",
  63: "Step in the rendering arc: from L62's opaque surface to a refractive one. A glass marble rolls across a domain-warped FBM cloud field — the same 'weather' pattern from L60. Each pixel that hits the sphere refracts twice (entry + exit, IOR 1.45) and samples the cloud field at the bent destination, so the field appears warped through the glass. A Fresnel term shifts the surface toward lime at grazing angles (the marble's rim), and a 3D fbm swirl is sampled inside the sphere for the colored ribbon a real marble has. Drag to roll the marble; tap to nudge.",
  64: "Another step in the rendering arc: L60 was a 2D fragment shader, L61–L63 added 3D geometry and refraction. L64 turns the WHOLE SCREEN into a flow visualization — no surface, no volume. Every pixel computes its own backward streamline through a 2D curl-noise vector field: 24 RK1 steps backward at h=0.02, accumulating brightness from a second fbm sampled along the trail. Curl-of-fbm gives an incompressible (divergence-free) field, which is why the result reads as flowing fabric — currents that swirl but never bunch up or vanish. The cursor adds a perpendicular vortex source, so dragging bends the streamlines locally. Tap = cream-to-lime pulse glow + the master lowpass opens (deep C2/G2/C3 drone underneath, pitched lower than L62 to feel like a real current). Follow the current.",
  65: "L60–L64 were all per-pixel-per-frame fragment shaders — deterministic functions of (x, y, t). L65 introduces actual STATE: each pixel's value depends on its previous value and its 8-neighbor average, updated by writing to one framebuffer while reading from another, then swapping. This is the classic ping-pong simulation pattern, and it's the unlock that lets pieces evolve over time rather than just be re-derived from coordinates. The chemistry is Gray-Scott reaction-diffusion (U + 2V → 3V), feed/kill coefficients chosen for 'mitosis' morphology — circular spots that form, grow, divide into two, drift apart, and start again. Drag injects activator (V) at the cursor — drawing seeds new growth wherever you go. Tap perturbs the kill coefficient briefly so the pattern morphology shifts (spots ⇄ maze) before it relaxes back. Warm A-minor ambient drone underneath. Let it grow.",
  66: "L65 had ONE state texture (a single chemistry field). L66 has TWO — and they interact. The first is the trail map (768×432, the same kind of texture L65 used). The second is the AGENT texture (128×128, encoding 16,384 particles' positions and headings, one per pixel). Each frame: every agent reads the trail map at three angles ahead, turns toward the strongest scent, walks forward, and the trail map then decays + diffuses + has new agent positions deposited additively. The result is a self-organizing vascular network — agents reinforce each other's paths until what emerges looks exactly like Physarum polycephalum solving a maze. Drag attracts (cursor biases each agent's heading); tap scatters a fresh ring of agents at the cursor that immediately joins the chase. C-minor drone underneath, slightly cooler than L65's A-minor.",
  67: "L65 gave the field state. L66 gave it agents. L67 gives it physics: a stable-fluids solver where five GPU textures — velocity, pressure, divergence, curl, ink — constrain each other every frame. The velocity field is forced to stay incompressible by twenty Jacobi relaxations of the pressure Poisson equation and a gradient subtraction, then everything is carried through the corrected flow semi-Lagrangian style. About thirty shader passes a frame, and none of them is the picture — the picture is just ink riding the result. Ink carries two channels, density and freshness, and freshness decays faster: whatever you just stirred is lime, and it cools to cream in a few seconds while the vortices you made keep turning. Drag to stir. Tap to drop. E-minor drone underneath; the lowpass opens with the stir.",
}
