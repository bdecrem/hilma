...
import { generateRun, todayDate, type NoonRun } from './generator'
...
// Today's run — load the baked JSON. Falls back to live generation if the
// file is missing (for dates we haven't baked yet).
const TODAY = todayDate()
const TODAYS_RUN: NoonRun = (bakedToday as NoonRun).date === TODAY
  ? (bakedToday as NoonRun)
  : generateRun(TODAY)
const TODAYS_MOOD = {
  name: TODAYS_RUN.mood.name,
  reason: TODAYS_RUN.mood.reason,
  palette: PALETTES[TODAYS_RUN.mood.palette],
  accent: ACCENTS[TODAYS_RUN.mood.accent],
}
...
export default function AmberNoon() {
  ...
  useEffect(() => {
    ...
    function nextBox(now: number, delay: number): Box {
      ...
      const attempt = TODAYS_RUN.attempts[idx] ?? TODAYS_RUN.winner
      attemptNumber++
        ...
        if (nP >= 0.85 || sE > 10) {
          ...
          if (succeed) {
            ...
            if (currentConcept) {
              ...
              if (statementRef.current) {
                statementRef.current.textContent = TODAYS_RUN.closingStatement
    ...
    const tick = () => {
      ...
      ctx.fillStyle = TODAYS_MOOD.palette.bg
  ...
  return (
    <div className="min-h-dvh overflow-hidden relative" style={{ backgroundColor: TODAYS_MOOD.palette.bg }}>
      <canvas
        ref={canvasRef}
