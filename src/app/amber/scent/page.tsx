...
export default function ScentPage() {
  ...
  useEffect(() => {
    ...
    const startAudio = async () => {
      ...
      // synth bus — direct to master, no bitcrush, no tilt LP. Just a
      // clean output bus for the chord stabs and ping voices.
      const sliceBus = ctx.createGain()
      sliceBus.gain.value = 0.7
      sliceBus.connect(master)
      sliceBusRef.current = sliceBus

      // sub bass — continuous C1 sine drone, bypasses bitcrush.
      // Same as chamber/current — proven, normal techno sub.
    ...
    const scheduleStep = (time: number, step: number, bar: number) => {
      ...
      // and now also carries the sub via its second sine layer)
      if (bar >= 2 && step % 4 === 0) {
        playKick(time, 0.65)
        const dt = (time - (audioCtxRef.current?.currentTime || time)) * 1000
        setTimeout(() => { kickGlowRef.current = 1 }, Math.max(0, dt))
      }
      // HIHAT — closed hat on every 16th from bar 6, no stutter
      if (bar >= 6) {
        const off = step % 4
        let amp = 0
        if (off === 0) amp = 0.04
        else if (off === 1) amp = 0.08
        else if (off === 2) amp = 0.12
        else if (off === 3) amp = 0.08
        if (amp > 0) playHat(time, amp)
      }
      // STAB — Cm9 chord on the offbeat (step 2 + step 10) starting bar 8.
      // Plays the buffer cleanly at native rate, no reverse, no stutter.
      if (bar >= 8 && (step === 2 || step === 10)) {
        playSlice(time, 'stab', { rate: 1.0, reverse: false, sliceStart: 0.0, gain: 0.55, stutter: 1 })
      }
      // PING — sparser higher melodic note, on step 6 every other bar starting bar 12
      if (bar >= 12 && bar % 2 === 0 && step === 6) {
        playSlice(time, 'ping', { rate: 1.0, reverse: false, sliceStart: 0.0, gain: 0.40, stutter: 1 })
      }
    }

    // Kick — copy-paste of chamber/current's proven values. Don't modify.
