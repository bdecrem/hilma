...
// keep the template below in sync with it). Unlike the rest of Polly's LLM
...
export async function setBookSummary(
  ...
  const { error } = await pollySupabase()
    ...
    .eq('id', threadId)
    .eq('user_id', userId)
  if (error) console.error('[polly/book-summary] setBookSummary failed:', error)
}

/// List-payload projection: status only, never the ~1,300-word markdown.
/// Clients fetch the full text via GET /api/polly/topics/[id]/book-summary.
...
function buildBookSummaryPrompt(thread: PollyThread): string {
...
export async function generateBookSummary(thread: PollyThread): Promise<void> {
  try {
    ...
    console.error('[polly/book-summary] generation failed:', message)
    await setBookSummary(thread.id, thread.user_id, {
      status: 'error',
