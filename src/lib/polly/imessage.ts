...
///      sends "Your Polly code is XXXXXX" via iMessage.
...
export async function startImessagePairing(
  ...
  const { error: upErr } = await sb
    .from('polly_imessage_pending')
    .upsert(
      { user_id: userId, handle, code, expires_at: expires },
      { onConflict: 'user_id,handle' },
    )
  if (upErr) {
    console.error('[polly/imessage] pending upsert failed:', upErr)
...
export async function confirmImessagePairing(
  ...
  if (!current.includes(handle)) {
    ...
    const { error } = await sb
      ...
      .eq('id', userId)
    if (error) {
      console.error('[polly/imessage] confirm persist failed:', error)
...
export async function removeImessageHandle(
  ...
  await sb.from('polly_users').update({ imessage_handles: next }).eq('id', userId)
