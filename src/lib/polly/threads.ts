...
export type PollyThreadMessage = {
  role: 'user' | 'assistant'
  text: string
  created_at: string
}
...
export type PollyAdditionalSource = {
...
export type PollyThread = {
  ...
  topic: string | null
  content: string | null
  additional_sources: PollyAdditionalSource[]
  quotes: PollyQuote[]
  messages: PollyThreadMessage[]
  created_at: string
  updated_at: string
  ...
  /** Narrated recap state (see lib/polly/audio-summary.ts). Null when the user
   ...
   *  lib/polly/book-summary.ts). Null when never generated. */
...
export function buildFullContent(thread: PollyThread): string {
  const parts: string[] = []
  if (thread.content) {
...
export function buildBudgetedContent(thread: PollyThread, maxChars: number): string {
  const appendix = buildFullContent({ ...thread, content: null } as PollyThread)
...
export function gatherUserNotes(thread: PollyThread): string {
  const parts: string[] = []
...
export async function createThread(
  ...
  if (error) {
    console.error('[polly] createThread failed:', error)
    return null
  }
...
export async function getLatestThread(
  ...
  if (error) {
    console.error('[polly] getLatestThread failed:', error)
    return null
  }
  return (data as PollyThread | null) ?? null
}
...
export async function getThreadById(
  ...
  if (error) {
    console.error('[polly] getThreadById failed:', error)
    return null
  }
  return (data as PollyThread | null) ?? null
}

export async function listTopicsForUser(userId: string): Promise<PollyThread[]> {
  const { data, error } = await pollySupabase()
    .from('polly_threads')
    .select('*')
    .eq('user_id', userId)
  ...
  if (error) {
    console.error('[polly] listTopicsForUser failed:', error)
    return []
  }
  return (data as PollyThread[]) ?? []
}
...
//   ★2 comes from flash sets (90%+ twice in a row — src/lib/polly/flash.ts)
...
export async function recordQuizStarted(
  ...
  if (error) console.error('[polly] recordQuizStarted failed:', error)
  return {
    stars: thread.stars,
    quiz_count: newCount,
...
export async function abandonPendingQuiz(
  ...
  const { error } = await pollySupabase()
    ...
    .eq('id', threadId)
    .eq('user_id', userId)
  if (error) console.error('[polly] abandonPendingQuiz failed:', error)
}
...
export async function completeQuiz(thread: PollyThread): Promise<RecordedQuiz> {
  ...
  const { error } = await pollySupabase()
    ...
    .eq('id', thread.id)
    .eq('user_id', thread.user_id)
  if (error) console.error('[polly] completeQuiz failed:', error)
  return {
    stars: newStars,
    quiz_count: thread.quiz_count,
    hard_quiz_completed_at: newHardAt,
...
export async function appendMessages(
  threadId: string,
  userId: string,
  existing: PollyThreadMessage[],
  toAdd: PollyThreadMessage[],
): Promise<void> {
  ...
  if (error) console.error('[polly] appendMessages failed:', error)
}
...
export async function setAdditionalSources(
  ...
  const { error } = await pollySupabase()
    ...
    .eq('id', threadId)
    .eq('user_id', userId)
  if (error) console.error('[polly] setAdditionalSources failed:', error)
}
...
export async function setStudyFocus(
  ...
  const { error } = await pollySupabase()
    ...
    .eq('user_id', userId)
  if (error) {
    console.error('[polly] setStudyFocus failed:', error)
    return false
  }
  return true
}
...
export async function appendQuote(
  ...
  const entry: PollyQuote = { text, author: author ?? null, created_at: new Date().toISOString() }
  ...
  const { error } = await pollySupabase()
    ...
    .eq('user_id', thread.user_id)
  if (error) {
    console.error('[polly] appendQuote failed:', error)
    return null
  }
  return next.length
}
...
export async function deleteQuoteAt(
  ...
  const { error } = await pollySupabase()
    ...
    .eq('user_id', userId)
  if (error) {
    console.error('[polly] deleteQuoteAt failed:', error)
    return false
  }
  return true
}
...
export async function getPendingQuote(userId: string): Promise<string | null> {
  const { data, error } = await pollySupabase()
    ...
    .maybeSingle()
  if (error) {
    console.error('[polly] getPendingQuote failed:', error)
    return null
  }
...
export async function setPendingQuote(
  ...
  const { error } = await pollySupabase()
    ...
    .eq('id', userId)
  if (error) console.error('[polly] setPendingQuote failed:', error)
}
...
export async function matchTopicByName(
  ...
  const labelOf = (t: PollyThread) => (t.topic ?? t.url ?? '').trim().toLowerCase()
