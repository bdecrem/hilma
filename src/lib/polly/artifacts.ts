// Polly artifacts — keepsakes the learner saves, quotes first. Loosely tied to
...
type ArtifactRow = {
  ...
  image_url: string | null
  created_at: string
  polly_threads: { topic: string | null } | null
}

function toArtifact(r: ArtifactRow): PollyArtifact {
  return {
    id: r.id,
...
export async function listArtifacts(userId: string): Promise<PollyArtifact[]> {
  const { data, error } = await pollySupabase()
    .from('polly_artifacts')
    .select('id, thread_id, kind, body, source, image_url, created_at, polly_threads(topic)')
    .eq('user_id', userId)
    .order('created_at', { ascending: false })
  if (error) {
    console.error('[polly/artifacts] list failed:', error)
    return []
  }
...
export async function createArtifact(
  ...
  const { data, error } = await pollySupabase()
    ...
    .single()
  if (error) {
    console.error('[polly/artifacts] create failed:', error)
    return null
  }
...
export async function updateArtifact(
  ...
  const { error } = await pollySupabase()
    ...
    .eq('id', id)
  if (error) {
    console.error('[polly/artifacts] update failed:', error)
    return false
  }
  return true
}
...
export async function deleteArtifact(
  ...
  const { data, error } = await sb
    ...
    .select('image_url')
  if (error) {
    console.error('[polly/artifacts] delete failed:', error)
    return false
  }
  ...
  if (path) {
    ...
    if (rmErr) console.error('[polly/artifacts] image remove failed:', rmErr)
  }
  return true
}
