// URL detection + fetching + HTML→text extraction for Polly.
...
const USER_AGENT =
  'Mozilla/5.0 (compatible; PollyBot/1.0; +https://hilma-nine.vercel.app)'
...
// datacenter ranges. Called by the /api/polly/youtube-transcript route which
...
export async function fetchYouTubeTranscriptLocal(
  ...
  try {
    ...
    if (speech.length < 200) {
      console.error(`[polly] YouTube transcript for ${videoId} is markers-only (${text.length} chars) — treating as none`)
      return null
    }
    return text
  } catch (err) {
    console.error(`[polly] YouTube transcript fetch (local) failed for ${videoId}:`, err)
    return null
  }
}
...
export async function fetchYouTubeTranscript(
  ...
  if (!secret) {
    console.error('[polly] F2_YOUTUBE_FETCH_URL set but F2_YOUTUBE_FETCH_SECRET missing')
    return null
  }
  ...
  try {
    ...
    if (!res.ok) {
      console.error(`[polly] YouTube proxy ${proxyBase} → ${res.status}`)
      return null
    }
    const data = (await res.json()) as { text?: string; error?: string }
    return data.text?.trim() || null
  } catch (err) {
    console.error(`[polly] YouTube proxy error:`, err)
    return null
  }
}
...
export async function fetchUrlContent(url: string): Promise<FetchedUrl> {
  ...
  if (ytId) {
    ...
    console.log(`[polly] YouTube ${ytId}: no transcript, falling back to HTML`)
  }
  ...
  try {
    ...
    if (!res.ok) {
      console.error(`[polly] fetch ${url} → ${res.status}`)
      return { body: null, title: null }
    }

    const ct = res.headers.get('content-type') || ''
    if (!ct.includes('text/html') && !ct.includes('text/plain')) {
      console.log(`[polly] skipping non-text content-type for ${url}: ${ct}`)
      return { body: null, title: null }
    }
    ...
    console.error(`[polly] fetch error for ${url}:`, err)
    return { body: null, title: null }
  }
}
