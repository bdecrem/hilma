...
export async function getUserProgress(userId: string): Promise<PollyProgress> {
  ...
  const [{ data, error }, { data: userRow }] = await Promise.all([
    ...
    pollySupabase().from('polly_users').select('xp').eq('id', userId).maybeSingle(),
  ])

  if (error) {
    console.error('[polly] getUserProgress failed:', error)
    return {
      level: 0,
      topic_count: 0,
      total_stars: 0,
