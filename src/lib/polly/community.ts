...
import type { PollyThread } from './threads'
...
export async function listCommunityTopics(): Promise<CommunityTopicRow[]> {
  const { data, error } = await pollySupabase()
    .from('polly_community_topics')
    .select('id, thread_id, shared_at, polly_threads(topic, url, kind), polly_users(username)')
    .order('shared_at', { ascending: false })
  if (error) {
    console.error('[polly/community] list failed:', error)
    return []
  }
  type Row = {
    id: string
    thread_id: string
    shared_at: string
    polly_threads: { topic: string | null; url: string | null; kind: string | null } | null
    polly_users: { username: string } | null
  }
  ...
  return ((data ?? []) as unknown as Row[])
    .filter((r) => r.polly_threads !== null)
    .map((r) => ({
      id: r.id,
      thread_id: r.thread_id,
      shared_at: r.shared_at,
      topic: r.polly_threads?.topic ?? null,
      url: r.polly_threads?.url ?? null,
      kind: r.polly_threads?.kind ?? null,
      author: r.polly_users?.username ?? '(unknown)',
    }))
}
...
export async function sharedThreadIds(userId: string): Promise<Set<string>> {
  const { data, error } = await pollySupabase()
    ...
    .eq('user_id', userId)
  if (error) {
    console.error('[polly/community] sharedThreadIds failed:', error)
    return new Set()
  }
...
export async function shareThread(userId: string, threadId: string): Promise<boolean> {
  ...
  if (error) {
    console.error('[polly/community] share failed:', error)
    return false
  }
  return true
}
...
export async function unshareThread(userId: string, threadId: string): Promise<boolean> {
  const { error } = await pollySupabase()
    ...
    .eq('user_id', userId)
  if (error) {
    console.error('[polly/community] unshare failed:', error)
    return false
  }
  return true
}
...
export async function forkCommunityTopic(
  ...
  const { data: entry, error: entryErr } = await db
    ...
    .maybeSingle()
  if (entryErr || !entry) {
    if (entryErr) console.error('[polly/community] fork lookup failed:', entryErr)
    return null
  }
  ...
  const { data: orig, error: origErr } = await db
    ...
    .maybeSingle()
  if (origErr || !orig) {
    if (origErr) console.error('[polly/community] fork thread load failed:', origErr)
    return null
  }
  const source = orig as PollyThread
  ...
  const { data: fresh, error: insertErr } = await db
    ...
    .single()
  if (insertErr || !fresh) {
    console.error('[polly/community] fork insert failed:', insertErr)
    return null
  }
  ...
  if (cardsErr) {
    console.error('[polly/community] fork cards load failed:', cardsErr)
  } else if (cards && cards.length > 0) {
    const { error: copyErr } = await db.from('polly_flash_cards').insert(
      cards.map((c) => ({
        user_id: userId,
    ...
    if (copyErr) console.error('[polly/community] fork cards copy failed:', copyErr)
  }
