...
import { buildFullContent, gatherUserNotes, type PollyThread } from './threads'
import { llmComplete } from './llm'

// Audio Summary — a one-shot narrated recap of a topic, stored as MP3 in the
// public f2-audio bucket and tracked in the polly_threads.audio_summary jsonb
...
export async function setAudioSummary(
  ...
  const { error } = await pollySupabase()
    ...
    .eq('id', threadId)
    .eq('user_id', userId)
  if (error) console.error('[polly] setAudioSummary failed:', error)
}
...
/// small. Full transcripts come from GET /api/polly/topics/[id]/summaries.
