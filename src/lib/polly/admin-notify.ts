...
export async function notifyAdminNewAccount(
  ...
  try {
    ...
    if (!handle) return
    const label = kind === 'guest' ? 'guest (Try Polly)' : 'signup'
    await sendIMessage({
      addresses: [handle],
      text: `\u{1F99C} New Polly account: ${username} — ${label}`,
    })
  } catch (err) {
    console.error('[polly/admin-notify] new-account ping failed:', err)
  }
}
