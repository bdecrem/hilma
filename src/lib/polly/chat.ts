...
export function threadHasVideos(thread: PollyThread | null): boolean {
...
function historyOf(thread: PollyThread): { role: 'user' | 'assistant'; content: string }[] {
...
export async function acknowledgeReflectionAnswer(
  ...
  const system = `You are Polly — a learning companion. The user just answered the single reflection question you asked them about: ${subject}.
