...
async function recordOutbound(args: SendArgs): Promise<void> {
  try {
    ...
    console.error('[polly/imessage] outbound record failed:', e)
  }
}
...
export async function isRecentOutbound(text: string): Promise<boolean> {
  ...
  const { data, error } = await pollySupabase()
    ...
    .limit(1)
  if (error) {
    console.error('[polly/imessage] outbound lookup failed:', error)
...
export async function sendIMessage(args: SendArgs): Promise<void> {
  ...
  try {
    ...
    console.error('[polly/imessage] agent send failed, falling back to BlueBubbles:', e)
  }
