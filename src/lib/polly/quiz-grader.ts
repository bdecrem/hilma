...
import { buildBudgetedContent, type PollyThread } from './threads'
...
export async function gradeQuizTwo(thread: PollyThread): Promise<QuizTwoGrade> {
...
Count how many of the 4 substantive answers (questions 2–5) were acceptable.`
  ...
  try {
    ...
    console.error('[polly] gradeQuizTwo failed — falling back to pass:', e)
    return { accepted: 3, total: 4, passed: true, notes: 'Grader unavailable — credit awarded.' }
  }
}

function recentTranscript(thread: PollyThread): string {
  const msgs = thread.messages.slice(-MAX_TRANSCRIPT_MESSAGES)
  return msgs
    .map(m => `${m.role === 'user' ? 'USER' : 'Polly'}: ${m.text.trim()}`)
    .join('\n\n')
}
