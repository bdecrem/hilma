// Polly "new short|medium|long" (and legacy "setup:") — agentic explainer-video
// finder.
//
...
async function planSearch(
  ...
  try {
    ...
    console.error('[polly] planSearch failed:', err)
    return null
  }
}
...
async function searchVideoIds(
  ...
  try {
    ...
    if (!res.ok) {
      console.error(`[polly] youtube search → ${res.status}: ${await res.text()}`)
      return []
    }
    ...
    console.error('[polly] youtube search error:', err)
    return []
  }
}
...
async function fetchVideoDetails(ids: string[]): Promise<VideoCandidate[]> {
  ...
  try {
    ...
    if (!res.ok) {
      console.error(`[polly] youtube videos → ${res.status}: ${await res.text()}`)
      return []
    }
    const data = await res.json()
    ...
    console.error('[polly] youtube videos error:', err)
    return []
  }
}
...
async function rankPicks(
  ...
  try {
    ...
    console.error('[polly] rankPicks failed:', err)
    return null
  }
}
