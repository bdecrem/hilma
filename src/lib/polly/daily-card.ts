...
/// link — opens the Peck tab in Polly, falls back to the web app.
// Polly has no domain yet, so no universal link: the custom scheme opens the
// app on the Peck tab. Revisit when Polly gets its own domain + AASA.
const PECK_URL = 'polly://peck'
...
export const IMESSAGE_DAILY_ONLY_REPLY =
  `Polly only does the daily card over iMessage. For chat, topics and everything else, open the app: ${PECK_URL}`
...
export async function recertPostscript(userId: string): Promise<string> {
  ...
  const { data: prefs } = await pollySupabase()
    .from('polly_users')
  ...
  const { data: due } = await pollySupabase()
    .from('polly_threads')
  ...
  return `\n\nP.S. ${lines.join(', and ')} — a 5-minute refresher in Polly keeps it gold.`
}
...
> {
  const { data: users, error } = await pollySupabase()
    .from('polly_users')
    .select('id, username, imessage_handles, daily_chat_guid')
    .eq('daily_card_enabled', true)
  if (error) {
    console.error('[polly/daily-card] user query failed:', error)
    return []
  }
  ...
  for (const u of (users ?? []) as {
    ...
    try {
      ...
      if (!u.daily_chat_guid && !handle) {
        ...
        console.error(`[polly/daily-card] ${u.username} has the daily card ON but no paired iMessage handle — nothing sent`)
      ...
      const text = `🃏 Polly daily card${label ? ` — ${label}` : ''}:
...
Reply with your answer (your own words are fine).${recertPs}`
      ...
      console.error(`[polly/daily-card] send failed for ${u.username}:`, detail)
...
async function setDailyState(userId: string, state: DailyState | null): Promise<void> {
  await pollySupabase().from('polly_users').update({ daily_card: state }).eq('id', userId)
}
...
async function addXp(userId: string, amount: number): Promise<number | null> {
  const { data, error } = await pollySupabase().rpc('polly_add_xp', {
    p_user_id: userId,
    p_amount: amount,
  })
...
export async function maybeHandleDailyAnswer(
  ...
  const { data: userRow } = await pollySupabase()
    .from('polly_users')
    .select('daily_card')
    .eq('id', userId)
    .maybeSingle()
...
async function sendBonusQuestion(userId: string): Promise<string | null> {
  ...
  if (!card) {
    await setDailyState(userId, null)
    return `No more cards to quiz today — keep playing in Polly: ${PECK_URL}`
  }
...
async function gradeBonusAnswer(
  ...
  const head = correct ? '✅ Right.' : `❌ Not quite — the answer: ${card.answer}.`
  const tail = `${xpTail(xp, total)}${peckNag(bonusStreak)} Both answers count on your Peck map — keep going in Polly: ${PECK_URL}`
