...
/// (gpt-live-1, Polly's voice sessions — probed 2026-09-11). The first eight
...
/// Preview clips for all of these ship in the Polly app bundle — regenerate
...
/// (realtimeVoice() for Polly sessions, walkVoice() for Peri walks).
export async function getVoicePrefs(userId: string): Promise<VoicePrefs> {
  const { data, error } = await pollySupabase()
    ...
    .maybeSingle()
  if (error || !data) {
    if (error) console.error('[polly/realtime] getVoicePrefs failed:', error)
...
export async function saveVoicePrefs(userId: string, prefs: VoicePrefs): Promise<boolean> {
  const { error } = await pollySupabase()
    ...
    .eq('id', userId)
  if (error) console.error('[polly/realtime] saveVoicePrefs failed:', error)
  return !error
}
...
function summarizeThreadForPrompt(thread: PollyThread): string {
...
export function buildRealtimeInstructions(input: {
  ...
  const base = `You are Polly, a learning companion, speaking in a live voice conversation with ${name}. Address them by their first name when it feels natural — do not use it in every sentence.
...
Grounding:
- If the user asks about saved Polly material, use get_topic_context before making specific claims.
...
You are currently in global voice mode. The user may ask about any saved Polly topic. Search/list-topic tools may be added later; for now, ask the user to open a topic for source-grounded discussion if you do not have enough context.`
}
...
export function buildFlashVoiceInstructions(input: {
  ...
  return `You are Polly, running a spoken flash-card round with ${name} ${scope}. You speak first.
...
export function buildFinalReviewInstructions(input: {
  ...
  const name = friendlyName(input.userName)
  return `You are Polly, conducting ${name}'s FINAL REVIEW — a spoken oral exam on a topic they have been studying. Passing at the highest level earns their mastery star, so be thorough and fair. You speak first.
...
export function buildSecondChanceInstructions(input: {
  ...
  return `You are Polly, giving ${name} their SECOND CHANCE — a short spoken retake after a Final Review that fell just short. Exactly THREE questions. Their mastery star is on the line: to pass, their three answers together must be A-level. You speak first.
...
export function buildRecertInstructions(input: {
  ...
  const name = friendlyName(input.userName)
  const weak = (input.weaknesses ?? []).filter((w) => w.trim())
  return `You are Polly, giving ${name} a quick REFRESHER on a topic they mastered a while ago — the check that keeps their gold badge shining. Exactly THREE questions, about five minutes. This is a retention check, not the original exam: warm, brisk, and confidence-building. You speak first.
...
export function getTopicContextTool() {
  return {
    ...
    description:
      'Fetch source-grounded context for the current Polly topic. Use before making specific claims about saved topic content.',
    parameters: {
      type: 'object',
      properties: {
        thread_id: {
          type: 'string',
          description: 'The Polly thread id for the topic being discussed.',
        },
        query: {
          type: 'string',
...
export async function createVoiceSession(input: {
  ...
  if (error) {
    console.error('[polly/realtime] createVoiceSession failed:', error)
    return null
  }
...
export async function updateVoiceSessionRealtimeId(input: {
  ...
  if (error) console.error('[polly/realtime] update realtime id failed:', error)
}
...
export async function finishVoiceSession(input: {
  ...
  if (error) {
    console.error('[polly/realtime] finishVoiceSession failed:', error)
    return false
  }
  return true
}
...
export async function getTopicContext(input: {
  ...
  recent_messages: PollyThreadMessage[]
  context: string
}> {
...
export async function createOpenAIRealtimeClientSecret(input: {
  instructions: string
  /** Override the tool catalog (defaults to Polly's get_topic_context). */
  tools?: unknown[]
  ...
  if (!res.ok) {
    console.error('[polly/realtime] OpenAI client secret failed:', json)
