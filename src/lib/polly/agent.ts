// Client-agnostic core entrypoint for Polly.
...
export type PollyClient = 'imessage' | 'web' | 'ios' | 'sms'

export type PollyMessage = {
  userId: string
  handle: string
  text: string
  client: PollyClient
   ...
   *  (Polly for Macintosh). */
  newTopic?: boolean
  /** Chat-model registry key (see lib/polly/llm.ts). Sent by the iOS/macOS
...
export type PollyReply = {
   ...
   *  has been marked `generating` and the caller (the /api/polly/messages route)
  ...
  /** Set by Polly agent's write_document action: the document is being
   *  written in the background (web search + long generation can outrun the
   *  request). The /api/polly/messages route runs the job via after(). */
  write_document?: {
    thread_id: string
    title: string
    brief: string
...
export async function processMessage(input: PollyMessage): Promise<PollyReply> {
  ...
  // generation runs in the background (see the /api/polly/messages route).
  ...
  // "polly <instruction>" → the content agent: act on the active topic's
  ...
  // address Polly fall through to a normal chat answer.
  const pollyMatch = text.match(/^polly\b[\s:,;.\-—!]*/i)
  if (pollyMatch) {
    const instruction = text.slice(pollyMatch[0].length).trim()
    return handlePollyCommand(userId, threadId, text, instruction, model)
  }
...
async function fileQuote(
  ...
  if (total === null) {
    return { reply: "Polly: couldn't save that quote. Try again in a sec." }
  }
  const label = thread.topic ?? thread.url ?? 'this topic'
  const by = author ? ` (${author})` : ''
  const reply = `Polly saved that quote${by} to "${label}". ${total} quote${total === 1 ? '' : 's'} on this topic now.`
...
async function handleQuote(
  ...
  if (!quoteText) {
    return { reply: 'Polly: add the quote text after "quote" — e.g. quote "Whereof one cannot speak…" (Wittgenstein).' }
  }

  if (threadId) {
    const thread = await getThreadById(userId, threadId)
    if (!thread) return { reply: "Polly: couldn't find that topic to attach the quote to." }
  ...
  const list = recent ? `\n\nRecent topics:\n${recent}` : ''
  return {
    reply: `Polly: which topic should I save this quote to? Reply with a topic name, or "new <name>" to create one.${list}`,
  }
}
...
async function resolvePendingQuote(
  ...
  if (/^cancel\b/i.test(answer)) {
    ...
    return { reply: 'Polly: okay, dropped that quote.' }
  }
  ...
  if (newMatch) {
    ...
    if (!name) {
      return { reply: 'Polly: give the new topic a name — e.g. new The Iliad.' }
    }
    const thread = await createThread({ userId, client, handle, topic: name })
    if (!thread) {
      return { reply: "Polly: couldn't create that topic. Try again in a sec." }
    }
    await setPendingQuote(userId, null)
  ...
  if (!match) {
    return {
      reply: `Polly: couldn't find a topic called "${answer}". Reply with an exact topic name, "new <name>" to create one, or "cancel" to drop it.`,
    }
  }
  await setPendingQuote(userId, null)
...
async function handleVideoSetup(
  ...
  if (!request) {
    return { reply: `Polly: name the topic — e.g. new ${band} quantum physics.` }
  }
  ...
  try {
    ...
    if (!result || result.picks.length === 0) {
      return {
        reply: `Polly: couldn't find solid ${bandLabel(band)} videos for that. Try rephrasing or narrowing the topic.`,
      }
    }
    ...
    if (!thread) {
      return { reply: 'Polly: found the videos but couldn\'t create the topic. Try again in a sec.' }
    }

    const now = new Date().toISOString()
    const extra: PollyAdditionalSource[] = picks.slice(1).map((p, i) => ({
      url: p.url,
      title: p.title,
    ...
    const reply =
      `Polly set up "${result.topicTitle}" with ${picks.length} video${picks.length === 1 ? '' : 's'}:\n\n${lines}\n\n` +
    ...
    console.error('[polly] handleVideoSetup failed:', err)
    return { reply: 'Polly: hit a snag setting that up (the video search or transcripts failed). Try again in a moment.' }
  }
}

// ---------------------------------------------------------------------------
// "polly <instruction>" — the content agent.

// Polly agent's client-side tools (the web_search server tool is added
...
function matchQuote(
  quotes: PollyArtifact[],
  raw: string,
): { quote: PollyArtifact } | { error: string } {
...
function quotesListingText(quotes: PollyArtifact[]): string {
...
function matchContextSource(
  sources: PollyAdditionalSource[],
  title: string,
): { index: number; source: PollyAdditionalSource } | { error: string } {
...
function listSourcesText(sources: PollyAdditionalSource[]): string {
...
/// "polly <instruction>" — an agent loop over the active topic's materials.
...
async function handlePollyCommand(
  ...
  const system = `You are Polly — the content agent for the learning topic "${label}". The user routes messages to you by starting them with "polly". You manage this topic's study materials, and you work like a careful assistant: read before you edit, verify what you're unsure of, apply the user's decisions, then report back.

How to work:
- This is a conversation. When acting needs a decision only the user can make, ask briefly instead of guessing — their next "polly ..." message continues the exchange (you see recent chat).
...
${instruction}`
  ...
  let writeDoc: PollyReply['write_document']
  let reply = ''
  try {
...
): Promise<{ target: PollyThread; error?: undefined } | { target?: undefined; error: string }> {
...
/// Execute one client-side polly tool. `mutated` tells the loop to refetch
/// the thread so later tool calls see the new state.
export async function executePollyTool(
  thread: PollyThread,
  name: string,
  input: Record<string, unknown>,
  model?: string,
  ...
  switch (name) {
    ...
    case 'set_review_grade': {
      ...
      const { data: session } = await pollySupabase()
    ...
    case 'update_settings': {
      ...
      if (typeof input.recert_enabled === 'boolean') {
        await sb.from('polly_users').update({ recert_enabled: input.recert_enabled }).eq('id', thread.user_id)
      ...
      if (typeof input.daily_card_enabled === 'boolean') {
        ...
        await sb.from('polly_users').update({ daily_card_enabled: input.daily_card_enabled }).eq('id', thread.user_id)
    ...
    case 'get_progress': {
      ...
      const { data: reviews } = await pollySupabase()
        .from('polly_voice_sessions')
...
${setLines.length ? setLines.join('\n') : '(none played yet)'}`,
    ...
    case 'list_topics': {
      ...
      const lines = topics.map((t) => {
        ...
        if ((t as PollyThread & { peck_excluded?: boolean }).peck_excluded) bits.push('out of Peck')
...
async function appendGeneratedSource(
  ...
  const next: PollyAdditionalSource[] = [
...
/// The background half of Polly agent's write_document action. Runs in
...
export async function runWriteDocumentJob(
  ...
  try {
    ...
    if (!doc) {
      await done(`Polly: "${title}" came out empty — try rephrasing the ask.`)
      return
    }
    const words = doc.split(/\s+/).length
    await appendGeneratedSource(thread, title, doc, false)
    await done(`Polly: "${title}" is ready — ~${words} words, filed in Topic Context.`)
  } catch (e) {
    const message = e instanceof Error ? e.message : String(e)
    console.error('[polly/agent] write_document job failed:', message)
    await done(`Polly: writing "${title}" failed (${message.slice(0, 200)}) — try again.`)
  }
}
...
/// directive; the /api/polly/messages route runs the actual generation in the
...
async function handleSummaryCommand(
  ...
  const thread = threadId
    ...
    : await getLatestThread(userId)
  if (!thread) {
    return { reply: 'Polly: open a topic first, then type "summary" (optionally with how to change it, e.g. "summary make it longer" or "summary 40 minutes").' }
  }
  ...
  const reply =
    `Polly is regenerating the summary for "${label}"${how}. ` +
...
async function handleNewNone(
  ...
  if (!topicPhrase) {
    return { reply: 'Polly: name the topic — e.g. new none quantum physics.' }
  }
  ...
  try {
    ...
    console.error('[polly] new-none opening failed:', err)
  }
...
async function handleMoreVideos(
  ...
  try {
    ...
    if (!result || result.picks.length === 0) {
      return {
        reply: `Polly: couldn't find more solid ${bandLabel(band)} videos on this beyond what you have. Try narrowing the angle — e.g. "3 more on <subtopic>".`,
        thread_id: thread.id,
      }
    }
    ...
    const now = new Date().toISOString()
    const extra: PollyAdditionalSource[] = picks.map((p, i) => ({
      url: p.url,
      title: p.title,
    ...
    console.error('[polly] handleMoreVideos failed:', err)
    return {
      reply: 'Polly: hit a snag finding more videos (the search or transcripts failed). Try again in a moment.',
      thread_id: thread.id,
    }
  }
}
...
async function handleNewUrl(
  ...
  if (!thread) {
    return { reply: "Polly: couldn't save that URL. Try again in a sec." }
  }

  const reply = content
    ? `Polly got it. Stored ${url} (${content.length.toLocaleString()} chars). Ask me anything about it.`
    : `Polly stored ${url}, but couldn't pull readable text from it. You can still ask — I'll answer from general knowledge.`
  return { reply, thread_id: thread.id }
}
...
async function startReflectionTurn(
  ...
  try {
    ...
    console.error('[polly] reflection quiz failed to start:', err)
    return { reply: 'Polly: hit an error starting the reflection quiz. Try again in a moment.' }
  }

  if (!reply) {
    return { reply: "Polly: couldn't start the reflection quiz. Try again in a moment." }
  }
...
/// topic done, clear pending — all in this single round-trip. Polly's reply is
...
async function completeReflectionTurn(
  ...
  try {
    ...
    console.error('[polly] reflection acknowledgement failed:', err)
    ack = 'Got it.'
  }
  if (!ack) ack = 'Got it.'
  ...
  switch (act.kind) {
    ...
    case 'more_videos': {
      if (!thread) {
        ...
        return { reply: 'Polly: open a video topic first, then ask for more videos.' }
      }
