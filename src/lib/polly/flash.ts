// Polly flash cards — generation, set scoring, judges, XP, Jumbo career.
//
...
Create exactly ${n} flash cards.`
  ...
  const { data, error } = await pollySupabase()
    ...
    .select('*')
  if (error || !data) {
    console.error('[polly/flash] insert cards failed:', error)
...
Produce the finished card.`
  ...
  const { data, error } = await pollySupabase()
    ...
    .single()
  if (error || !data) {
    console.error('[polly/flash] authorFlashCard insert failed:', error)
...
export async function redoFlashCards(
  ...
  if (old.length > 0) {
    ...
    if (error) console.error('[polly/flash] redo delete-old failed:', error)
  }
  return newCards
}
...
export async function listFlashCards(
  ...
  if (error) {
    console.error('[polly/flash] listFlashCards failed:', error)
    return []
  }
  return (data as FlashCard[]) ?? []
}
...
export async function countPeckCards(userId: string): Promise<number> {
  ...
  const { data, error } = await pollySupabase()
    ...
    .eq('user_id', userId)
  if (error) {
    console.error('[polly/flash] countPeckCards failed:', error)
    return 0
  }
...
export async function countFlashCards(userId: string): Promise<number> {
  const { count, error } = await pollySupabase()
    .from('polly_flash_cards')
    .select('id', { count: 'exact', head: true })
    .eq('user_id', userId)
  if (error) {
    console.error('[polly/flash] countFlashCards failed:', error)
    return 0
  }
  return count ?? 0
}
...
export async function listDecks(userId: string): Promise<FlashDeck[]> {
  ...
  const [{ data: cardRows, error: cardErr }, { data: threadRows }] = await Promise.all([
    sb.from('polly_flash_cards').select('thread_id, rating').eq('user_id', userId),
    sb
      .from('polly_threads')
  ...
  if (cardErr) {
    console.error('[polly/flash] listDecks failed:', cardErr)
    return []
  }
...
export async function getFlashCardsByIds(
  ...
  const { data, error } = await pollySupabase()
    ...
    .in('id', ids)
  if (error) {
    console.error('[polly/flash] getFlashCardsByIds failed:', error)
    return []
  }
  return (data as FlashCard[]) ?? []
}
...
export async function updateFlashCard(
  ...
  const { data, error } = await pollySupabase()
    ...
    .maybeSingle()
  if (error) {
    console.error('[polly/flash] updateFlashCard failed:', error)
    return null
  }
...
export async function deleteFlashCard(
  ...
  const { error } = await pollySupabase()
    ...
    .eq('user_id', userId)
  if (error) {
    console.error('[polly/flash] deleteFlashCard failed:', error)
    return false
  }
  return true
}
...
async function applyReviews(
  ...
  await Promise.all(
    cards.map((card) => {
      ...
      return pollySupabase()
        .from('polly_flash_cards')
        .update({ ...next, updated_at: now.toISOString() })
        .eq('id', card.id)
        .eq('user_id', userId)
        .then(({ error }) => {
          if (error) console.error('[polly/flash] applyReviews failed:', error)
        })
    }),
  )
}
...
export async function markCardsShown(userId: string, cardIds: string[]): Promise<void> {
  ...
  const { error } = await pollySupabase().rpc('polly_mark_cards_shown', {
    p_user_id: userId,
    p_card_ids: cardIds,
    p_now: now,
  })
  if (error) console.error('[polly/flash] markCardsShown failed:', error)
}
...
async function judgeJson<T>(
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    console.error(`[polly/flash] judgeJson attempt ${attempt + 1} on ${model}: ${lastErr}`)
  }
...
export async function setPeckCredits(
  ...
  const { error } = await pollySupabase()
    ...
    .eq('id', userId)
  if (error) console.error('[polly/flash] setPeckCredits failed:', error)
}
...
export async function reviewSingleCard(
  ...
  const { error } = await pollySupabase()
    ...
    .eq('id', card.id)
    .eq('user_id', userId)
  if (error) console.error('[polly/flash] reviewSingleCard failed:', error)
}
...
export async function judgeVoiceSet(
  ...
  const convo = (transcript ?? [])
    .map((t) => `${t.role === 'user' ? 'USER' : 'Polly'}: ${(t.text ?? '').trim()}`)
    .filter((l) => l.length > 6)
    .join('\n')
    .slice(0, 400_000)
  if (!convo) return out
...
export async function judgeFinalReview(
  ...
  try {
    ...
    console.error('[polly/flash] verifyOutsideClaims failed:', e)
  }
...
export async function recordFlashSet(input: {
  ...
  const { data: setRow, error: setErr } = await sb
    ...
    .single()
  if (setErr || !setRow) {
    console.error('[polly/flash] recordFlashSet insert failed:', setErr)
  ...
  if (input.jumboLevel != null && total >= SET_SIZE) {
    try {
      ...
      console.error('[polly/flash] markPeckWeek failed:', e)
    }
  }
  ...
  try {
    ...
    console.error('[polly/flash] review scheduling failed:', e)
  }

  // XP — atomic increment (polly_add_xp) so concurrent sets from the same
  // account (phone + Mac) can't lose an update. Returns the new total.
  let totalXp = xp
  const { data: xpTotal, error: xpErr } = await sb.rpc('polly_add_xp', {
    p_user_id: input.userId,
    p_amount: xp,
  })
  if (xpErr || typeof xpTotal !== 'number') {
    console.error('[polly/flash] polly_add_xp failed:', xpErr)
...
export async function recordVoiceSessionGrade(
  ...
  const { error } = await pollySupabase()
    ...
    .eq('user_id', userId)
  if (error) console.error('[polly/flash] recordVoiceSessionGrade failed:', error)
}
...
export async function getSecondChanceState(
  ...
  if (error) {
    console.error('[polly/flash] getSecondChanceState failed:', error)
    return none
  }
...
export async function awardFinalReviewStar(
  ...
  const { error } = await pollySupabase()
    ...
    .eq('id', threadId)
    .eq('user_id', userId)
  if (error) console.error('[polly/flash] awardFinalReviewStar failed:', error)
}
...
export async function scheduleRecertDue(
  ...
  const { data, error } = await pollySupabase()
    ...
    .neq('id', threadId)
  if (error) {
    console.error('[polly/flash] scheduleRecertDue lookup failed:', error)
...
export async function applyRecertRenewal(
  ...
  const { error } = await pollySupabase()
    ...
    .eq('id', threadId)
    .eq('user_id', userId)
  if (error) console.error('[polly/flash] applyRecertRenewal failed:', error)
  const { error: xpErr } = await pollySupabase().rpc('polly_add_xp', {
    p_user_id: userId,
    p_amount: RECERT_XP,
  })
  if (xpErr) console.error('[polly/flash] recert XP failed:', xpErr)
  return dueAt
}
...
export async function listFlashSets(
  ...
  const { data, error } = await pollySupabase()
    ...
    .order('created_at', { ascending: false })
  if (error) {
    console.error('[polly/flash] listFlashSets failed:', error)
    return []
  }
...
export async function getJumboState(userId: string): Promise<JumboState> {
  ...
  const [{ data: userRow }, cardCount, { data: sets, error }, streak] = await Promise.all([
    sb.from('polly_users').select('xp').eq('id', userId).maybeSingle(),
  ...
  if (error) console.error('[polly/flash] getJumboState failed:', error)
...
export async function completePeckLevels(
  ...
  for (let l = 1; l <= target; l++) {
    ...
    const mode = jumboLevelMode(l)
    const { error } = await pollySupabase().from('polly_flash_sets').insert({
      user_id: userId,
      thread_id: null,
      jumbo_level: l,
      mode,
    ...
    if (error) {
      console.error('[polly/flash] completePeckLevels insert failed:', error)
      break
    }
    granted.push(l)
  }
