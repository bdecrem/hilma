...
enum PerfMeter {
    ...
    private static func tick() {
        ...
        print(String(format: "[perf] cpu %.0f%% of one core (%.0f%% of %d) · mem %.0f MB · %@ · %.0f fps",
                     cpu, cpu / cores, Int(cores), Double(footprint()) / 1_048_576, thermal, fps))
    }

    /// Sum of the threads' current CPU usage, in percent of one core.
