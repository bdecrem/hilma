/* aes128.c - portable AES-128 encryption (CTR mode).
 *
 * On-the-fly SubBytes via a 256-byte S-box; MixColumns via xtime. Only
 * 8-bit and 32-bit arithmetic. No 64-bit math, no compiler builtins.
 * Compiles cleanly under clang/gcc on a modern host and Retro68 gcc (68000).
 */
#include "aes128.h"
#include <string.h>

/* AES S-box (FIPS-197). */
static const uint8_t sbox[256] = {
  
};

/* Round constants for the key schedule. */
static const uint8_t rcon[10] = {
  0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1b,0x36
};

/* General Rijndael key expansion for Nk = 4 (AES-128) or 8 (AES-256). */
void aes_init(aes_ctx *c, const uint8_t *key, int keybytes)
{
  uint8_t *rk = c->rk;
  int nk = keybytes / 4;          /* 4 or 8 */
  int nr = nk + 6;                /* 10 or 14 */
  int total = 4 * (nr + 1);       /* total 32-bit words in the schedule */
  int i;
  uint8_t t[4];

  c->nr = nr;
  memcpy(rk, key, (size_t)keybytes);

  for (i = nk; i < total; i++) {
    uint8_t *prev = rk + (i - 1) * 4;
    t[0] = prev[0]; t[1] = prev[1]; t[2] = prev[2]; t[3] = prev[3];
    if (i % nk == 0) {
      /* RotWord + SubWord + Rcon */
      uint8_t tmp = t[0];
      t[0] = (uint8_t)(sbox[t[1]] ^ rcon[(i / nk) - 1]);
      t[1] = sbox[t[2]];
      t[2] = sbox[t[3]];
      t[3] = sbox[tmp];
    } else if (nk > 6 && i % nk == 4) {
      /* AES-256 only: an extra SubWord every 8 words */
      t[0] = sbox[t[0]]; t[1] = sbox[t[1]]; t[2] = sbox[t[2]]; t[3] = sbox[t[3]];
    }
    {
      uint8_t *w0 = rk + (i - nk) * 4;
      uint8_t *w  = rk + i * 4;
      w[0] = (uint8_t)(w0[0] ^ t[0]);
      w[1] = (uint8_t)(w0[1] ^ t[1]);
      w[2] = (uint8_t)(w0[2] ^ t[2]);
      w[3] = (uint8_t)(w0[3] ^ t[3]);
    }
  }
}

/* GF(2^8) multiply by 2 (the AES x-times operation). */
static uint8_t xtime(uint8_t x)
{
  return (uint8_t)((x << 1) ^ (((x >> 7) & 1) * 0x1b));
}

static void add_round_key(uint8_t s[16], const uint8_t *rk)
{
  int i;
  for (i = 0; i < 16; i++) s[i] = (uint8_t)(s[i] ^ rk[i]);
}

static void sub_bytes(uint8_t s[16])
{
  int i;
  for (i = 0; i < 16; i++) s[i] = sbox[s[i]];
}

/* State is column-major: byte index = row + 4*col. ShiftRows shifts row r
 * left by r positions. */
static void shift_rows(uint8_t s[16])
{
  uint8_t t;
  /* row 1: shift left by 1 */
  t = s[1]; s[1] = s[5]; s[5] = s[9]; s[9] = s[13]; s[13] = t;
  /* row 2: shift left by 2 (swap pairs) */
  t = s[2]; s[2] = s[10]; s[10] = t;
  t = s[6]; s[6] = s[14]; s[14] = t;
  /* row 3: shift left by 3 == right by 1 */
  t = s[15]; s[15] = s[11]; s[11] = s[7]; s[7] = s[3]; s[3] = t;
}

static void mix_columns(uint8_t s[16])
{
  int col;
  for (col = 0; col < 4; col++) {
    uint8_t *p = s + col * 4;
    uint8_t a0 = p[0], a1 = p[1], a2 = p[2], a3 = p[3];
    uint8_t all = (uint8_t)(a0 ^ a1 ^ a2 ^ a3);
    p[0] = (uint8_t)(a0 ^ all ^ xtime((uint8_t)(a0 ^ a1)));
    p[1] = (uint8_t)(a1 ^ all ^ xtime((uint8_t)(a1 ^ a2)));
    p[2] = (uint8_t)(a2 ^ all ^ xtime((uint8_t)(a2 ^ a3)));
    p[3] = (uint8_t)(a3 ^ all ^ xtime((uint8_t)(a3 ^ a0)));
  }
}

void aes_encrypt(const aes_ctx *c, const uint8_t in[16], uint8_t out[16])
{
  uint8_t s[16];
  int round;

  memcpy(s, in, 16);
  add_round_key(s, c->rk);

  for (round = 1; round < c->nr; round++) {
    sub_bytes(s);
    shift_rows(s);
    mix_columns(s);
    add_round_key(s, c->rk + round * 16);
  }

  /* Final round: no MixColumns. */
  sub_bytes(s);
  shift_rows(s);
  add_round_key(s, c->rk + c->nr * 16);

  memcpy(out, s, 16);
}

void aes_ctr_xor(const aes_ctx *c, uint8_t counter[16],
                 const uint8_t *in, uint8_t *out, size_t len)
{
  uint8_t ks[16];
  size_t i;
  int j;

  while (len > 0) {
    size_t n = len < 16 ? len : 16;
    aes_encrypt(c, counter, ks);
    for (i = 0; i < n; i++) out[i] = (uint8_t)(in[i] ^ ks[i]);
    in += n;
    out += n;
    len -= n;
    /* Increment counter as a 128-bit big-endian integer. */
    for (j = 15; j >= 0; j--) {
      if (++counter[j] != 0) break;
    }
  }
}
