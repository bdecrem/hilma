#!/usr/bin/env bash
# Build Macinclaude TalkingPlus for the Mac Plus with the Retro68 toolchain.
# Outputs TalkingPlus.bin / TalkingPlus.APPL / TalkingPlus.dsk into ./build/.
#
# Usage:
#   ./build.sh         normal build (real Plus; talks to the agent over serial)
#   ./build.sh test    OFFLINE TEST build for Mini vMac (-DTKM_TEST): no
...
if [ "${1:-}" = "test" ]; then
  EXTRA=(-DTKM_TEST=ON)
  echo "*** TEST BUILD (TKM_TEST): offline Mini vMac build - commands load the"
...
echo
echo "Built:"
ls -1 TalkingPlus.bin TalkingPlus.APPL TalkingPlus.dsk 2>/dev/null
