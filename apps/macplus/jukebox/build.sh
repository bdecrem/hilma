#!/usr/bin/env bash
# Build Macinclaude Jukebox for the Mac Plus with the Retro68 toolchain.
# Outputs Jukebox.bin / Jukebox.APPL / Jukebox.dsk into ./build/.
#
# Usage:
#   ./build.sh         normal build (real Plus; talks to the agent over serial)
#   ./build.sh test    OFFLINE TEST build for Mini vMac (-DJUKE_TEST): no
...
if [ "${1:-}" = "test" ]; then
  EXTRA=(-DJUKE_TEST=ON)
  echo "*** TEST BUILD (JUKE_TEST): offline Mini vMac build - commands load the"
...
echo
echo "Built:"
ls -1 Jukebox.bin Jukebox.APPL Jukebox.dsk 2>/dev/null
