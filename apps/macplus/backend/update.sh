...
UID_N="$(id -u)"
AGENT_DIRS="agent agent-atkinson agent-surf agent-mux agent-imessage agent-diag agent-quote agent-bridge agent-screen agent-porthole"
LONG_RUNNERS="mux imessage diag quote bridge screen porthole"
...
sleep 2
echo "== fleet status =="
for spec in code:2324 paint:2325 surf:2326 imessage:2328 mux:2330 diag:2331 quote:2332 bridge:2333 screen:2334 porthole:2336; do
  n="${spec%%:*}"; p="${spec##*:}"
