...
UID_N="$(id -u)"
ALL="code paint surf mux imessage diag quote bridge screen netspeed porthole pssh rsh pixel"
DEFAULT_BOOT="surf mux imessage diag quote bridge screen netspeed porthole pssh rsh pixel"
BOOT="${*:-$DEFAULT_BOOT}"
...
for n in $BOOT; do
  case "$n" in
    ...
    mux) p=2330;; diag) p=2331;; quote) p=2332;; bridge) p=2333;; screen) p=2334;; netspeed) p=2335;; pixel) p=2337;;
  esac
