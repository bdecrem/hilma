...
- Verify: compiling is necessary, not sufficient. Do (1) the build, and (2) a host-clang logic test wherever the logic is pure (e.g. sudoku/logic_test.c: clang it and run it). You CANNOT drive Mini vMac here - you have no computer-use MCP (only a browser/Playwright one), and the mini is headless so it can't even screenshot ("could not create image from display"). So do NOT try to open Mini vMac, or a Playwright/computer-use MCP, to test a Plus app - that just errors. For anything VISUAL, build the app and deliver it to the REAL Plus over the Bridge (queue the .bin, tell Bart to open The Bridge to receive) and have Bart look. See apps/macplus/CLAUDE.md "Verifying".
- Ship an app to the Plus: YOU can deliver it over The Bridge yourself - copy the built .bin into the mini's ~/bridge-outbox/ (you run as admin; the sh.macplus.bridge service on :2333 streams it and overwrites the app in place). BUT the Plus only receives while its Bridge app is OPEN, and the person is talking to you FROM Macinclaude Code, which IS the running app. So after you queue the .bin, tell them: quit Macinclaude, launch "The Bridge" on the Plus to receive it, then relaunch the app. Never claim it's installed until they confirm - it just sits in the outbox until the Plus Bridge connects. The Bridge app itself updates ONLY via SD card, never over itself. Backend/agent changes deploy by pushing to main, then running ~/hilma-deploy/apps/macplus/backend/update.sh on the mini.
