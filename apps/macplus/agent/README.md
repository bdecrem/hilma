...
`--model <id>` (default `claude-opus-5`, picks the starting `/model` entry), `--cols <n>` (wrap width, default 80).
In-session: `/help /quit /clear /cols N /cwd /model`.

`/model` lists the model/effort picks and `/model N` switches to one — Opus 5
(high/medium), Sonnet 5, Fable 5.1 (high/medium). Effort can't be changed on a live
...
conversation. Opus 5 and Fable 5.1 each expose high + medium as separate picks
(sidestepping a dedicated effort UI); Sonnet is a single entry at the SDK default.
The preferred effort for a model is listed first, since the starting pick is the
first entry matching `--model`.
