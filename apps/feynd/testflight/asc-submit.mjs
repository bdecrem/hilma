...
const KID="5A5HNSWA33", ISS="69a6de80-eb13-47e3-e053-5b8c7c11a4d1", APP="6773165027", VER=process.argv[2]; if(!VER){console.error("usage: node asc-submit.mjs <buildNumber>");process.exit(1)};
...
let build;
for(let i=0;i<40;i++){const d=await api(`/builds?filter[app]=${APP}&filter[version]=${VER}&filter[preReleaseVersion.platform]=IOS`);build=d.data[0];const st=build?.attributes.processingState;console.log("build 71:",st??"not yet");if(st==="VALID")break;if(st==="FAILED"||st==="INVALID")process.exit(1);await sleep(30000)}
...
const all=(await api(`/builds?filter[app]=${APP}&filter[preReleaseVersion.platform]=IOS&filter[expired]=false&include=buildBetaDetail&limit=20&sort=-uploadedDate`));
...
try{await api(`/betaAppReviewSubmissions`,"POST",{data:{type:"betaAppReviewSubmissions",relationships:{build:{data:{type:"builds",id:build.id}}}}});console.log("submitted 71 for beta review")}catch(e){console.log("submit:",e.message.slice(0,300))}
