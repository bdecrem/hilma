...
let build;
for(let i=0;i<40;i++){const d=await api(`/builds?filter[app]=${APP}&filter[version]=${VER}&filter[preReleaseVersion.platform]=IOS`);build=d.data[0];const st=build?.attributes.processingState;console.log("build "+VER+":",st??"not yet");if(st==="VALID")break;if(st==="FAILED"||st==="INVALID")process.exit(1);await sleep(30000)}
