...
- Harness account without a guest sign-up: log in as `pollytest-1789704474@example.com` / `pollytest-voice-0921` (`POST /api/polly/auth/login`, take the `polly_session` cookie, pass it as `--cookie`). Local `F2_SESSION_SECRET` is not production's, so a locally signed cookie gets 401 there. Six PASS lines against production through the mini's bridge on 2026-09-21.
