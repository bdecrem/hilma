...
enum PollyAPIError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .transport(let e):
            if let u = e as? URLError, Backend.isConnectionFailure(u) {
                return "Couldn't reach Polly securely (\(u.localizedDescription)). Check VPN, Wi-Fi, or cellular and try again."
            }
...
final class PollyAPI {
    static let shared = PollyAPI()
    ...
    struct LoginResponse: Codable { let user: PollyUser }
    ...
    struct TopicsResponse: Codable { let topics: [PollyTopic] }
    struct ThreadResponse: Codable { let thread: PollyThread }
    ...
    struct AudioSummaryStartResponse: Codable {
        let audioSummary: PollyAudioSummary?
    ...
    private struct BookSummaryResponse: Codable {
        let bookSummary: PollyBookSummary?
    ...
    func readSourceText(id: String, kind: String, index: Int) async throws -> SourceText {
        ...
        guard let http = response as? HTTPURLResponse else { throw PollyAPIError.http(0, "non-HTTP response") }
        if http.statusCode == 401 { throw PollyAPIError.unauthenticated }
        if http.statusCode >= 400 { throw PollyAPIError.http(http.statusCode, errorMessage(from: data, response: http)) }
    ...
    func uploadAvatar(imageData: Data, mime: String = "image/jpeg") async throws -> String {
        ...
        let boundary = "polly-\(UUID().uuidString)"
        ...
        do {
            ...
            throw PollyAPIError.transport(error)
        }
        guard let http = response as? HTTPURLResponse else {
            throw PollyAPIError.http(0, "non-HTTP response")
        }
        if http.statusCode == 401 { throw PollyAPIError.unauthenticated }
        if http.statusCode >= 400 {
            throw PollyAPIError.http(http.statusCode, errorMessage(from: data, response: http))
        }
        let decoded = try decoder.decode(AvatarResponse.self, from: data)
        guard let urlStr = decoded.avatarUrl else {
            throw PollyAPIError.http(500, "no avatar_url in response")
        }
        return urlStr
    }
    ...
    struct ArtifactsResponse: Codable { let artifacts: [PollyArtifact] }
    struct ArtifactResponse: Codable { let artifact: PollyArtifact }
    ...
    func createImageArtifact(imageData: Data, mime: String = "image/jpeg", body: String?, source: String?, threadId: String?) async throws -> PollyArtifact {
        ...
        guard let http = response as? HTTPURLResponse else { throw PollyAPIError.http(0, "non-HTTP response") }
        if http.statusCode == 401 { throw PollyAPIError.unauthenticated }
        if http.statusCode >= 400 { throw PollyAPIError.http(http.statusCode, errorMessage(from: respData, response: http)) }
        do {
            return try decoder.decode(ArtifactResponse.self, from: respData).artifact
        } catch {
            throw PollyAPIError.decode(error)
        }
    }
    ...
    private func postRaw<B: Encodable>(_ path: String, body: B) async throws -> Data {
        ...
        if http.statusCode == 401 {
            throw PollyAPIError.unauthenticated
        }
        if http.statusCode >= 400 {
            throw PollyAPIError.http(http.statusCode, errorMessage(from: data, response: http))
        }
