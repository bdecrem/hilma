...
struct PeckOddsSheet: View {
            ...
            ForEach(Self.weights, id: \.self) { w in
                ...
                Button { setWeight(deck, w) } label: {
                    Text(chipLabel(w))
                        ...
                        .foregroundStyle(selected ? Color(hex: 0x261C06) : PollyTheme.text2)
                        .frame(width: 32, height: 24)
                        .background(
                            selected ? PollyTheme.accent : PollyTheme.surface2,
                            in: Capsule(),
                        )
                }
                .buttonStyle(.plain)
    ...
    private func setWeight(_ deck: FlashDeck, _ w: Double) {
        ...
        Task { try? await PollyAPI.shared.setPeckWeight(id: deck.threadId, weight: w) }
    }
