...
struct FlashTabView: View {
    ...
    @State private var bannerTopics: [PollyTopic] = []
    ...
    var body: some View {
        ...
        .task {
            ...
            if bannerTopics.isEmpty,
               let cachedTopics: [PollyTopic] = ScreenCache.load(key: ScreenCache.topics) {
                bannerTopics = cachedTopics
            }
            Task { bannerTopics = (try? await PollyAPI.shared.listTopics()) ?? bannerTopics }
            await load()
        ...
        VStack(spacing: 16) {
            ...
            HStack(spacing: 4) {
                ForEach(0..<10, id: \.self) { i in
                    Capsule()
                        .fill(i < state.cardCount ? PollyTheme.accent : PollyTheme.surface2)
    ...
    private func load() async {
        ...
        defer { loading = false }
        do {
            state = try await PollyAPI.shared.jumboState()
    ...
    private func play(_ level: JumboLevelInfo, mode: String) {
        ...
        Task {
            do {
                let start = try await PollyAPI.shared.startJumboSet(level: level.level, mode: mode)
                sheetLevel = nil
...
/// dusk in dark.
enum PeckPalette {
    static let skyA      = PollyTheme.adaptiveColor(dark: 0x1E2440, light: 0xAEE0EE)
    static let skyB      = PollyTheme.adaptiveColor(dark: 0x4A3D63, light: 0xEAF7EF)
    static let sunMoon   = PollyTheme.adaptiveColor(dark: 0xF3E3B2, light: 0xFFD469)
    static let cloud     = PollyTheme.adaptiveColor(dark: 0x4A4468, light: 0xFFFFFF)
    static let sea       = PollyTheme.adaptiveColor(dark: 0x2C5B66, light: 0x79C6C4)
    static let island    = PollyTheme.adaptiveColor(dark: 0x22453A, light: 0x4E8F6E)
    static let palm      = PollyTheme.adaptiveColor(dark: 0x2E5B3F, light: 0x4E8F6E)
    static let trunk     = PollyTheme.adaptiveColor(dark: 0x4A3A2C, light: 0x8A6A4C)
    static let hillFar   = PollyTheme.adaptiveColor(dark: 0x3A5A4A, light: 0xBFDCA4)
    static let hillMid   = PollyTheme.adaptiveColor(dark: 0x2E4A3A, light: 0x96C77E)
    static let hillNear  = PollyTheme.adaptiveColor(dark: 0x254032, light: 0x7BB662)
    static let grass     = PollyTheme.adaptiveColor(dark: 0x1D3528, light: 0x5FA24C)
    static let tuft      = PollyTheme.adaptiveColor(dark: 0x2E4A38, light: 0x4C8C3D)
    static let tree1     = PollyTheme.adaptiveColor(dark: 0x2E5B3F, light: 0x5F9E4C)
    static let tree2     = PollyTheme.adaptiveColor(dark: 0x3A6B4A, light: 0x7BB662)
    static let pathDotColor = PollyTheme.adaptiveColor(dark: 0xD9C89A, light: 0xFFFDF4)
    ...
    static let nodeCurFace = PollyTheme.adaptiveColor(dark: 0x3B3560, light: 0xFFFDF4)
    static let nodeCurNum  = PollyTheme.adaptiveColor(dark: 0xFFF6E0, light: 0x3E4A52)
    static let startText   = PollyTheme.adaptiveColor(dark: 0xF6B04E, light: 0x8A5B14)
    static let nodeLock    = PollyTheme.adaptiveColor(dark: 0x332F4E, light: 0xC4D6C8)
    static let nodeLockIcon = PollyTheme.adaptiveColor(dark: 0x5C567E, light: 0x8FA695)
    static let starGold    = PollyTheme.adaptiveColor(dark: 0xF6B04E, light: 0xF0A830)
    static let starDim     = PollyTheme.adaptiveColor(dark: 0x4A4468, light: 0xC9D6CB)
}
            ...
            VStack(spacing: 14) {
                    ...
                    HStack(spacing: 3) {
                        ForEach(0..<3, id: \.self) { s in
                            Image(systemName: "star.fill")
                                ...
                                .foregroundStyle(s < level.stars ? PollyTheme.gold : PollyTheme.text4)
                        }
                    }
                }
                ...
                VStack(spacing: 10) {
                    ...
                    modeButton("voice", icon: "mic.fill", title: "Voice round",
                               sub: "Polly quizzes you out loud — clears at \(jumboPassScore(mode: "voice"))/10")
                }
