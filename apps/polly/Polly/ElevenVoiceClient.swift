...
///   turn to our voice bridge → /api/polly/eleven/turn. The phone never sees the
///   prompt; it only names the session with the `dodo_voice_session` variable.
/// - Polly opens a session because the client sends a kickoff text
...
///   learner cuts in, a correction carries what Polly actually got to say — that
///   is what the transcript keeps.
@MainActor
@Observable
final class ElevenVoiceClient: PollyVoiceClient {
    ...
    private var sessionResponse: PollyAPI.ElevenSessionResponse?
    ...
    /// Hold-to-talk: the button went down. Polly is told to stop (ElevenLabs
    ...
    /// Test rig: cumulative energy + seconds of audio RECEIVED from Polly,
    ...
    func start() async {
        ...
        do {
            ...
            if agentReady { openSession() } else { status = "Waiting for Polly..." }
        } catch {
    ...
    /// never turns it off; the next thing to touch the audio session (a sound
    /// effect setting `.ambient`) breaks that engine and the next voice session
    /// fails to start with "Audio engine returned error code: -3001" (seen in
    /// Dodo 2026-09-22). Release it.
    ...
    /// send the kickoff so Polly speaks first. Runs once.
    ...
    /// is tried again when Polly first speaks.
    ...
    /// The learner cut in: keep what Polly actually got to say.
    ...
    private func finishSession() async {
        ...
        do {
            try await PollyAPI.shared.finishLiveSession(
                id: id,
                transcript: rows,
