...
struct FlashCardsView: View {
    ...
    private var header: some View {
        HStack(alignment: .center) {
            IconCircleButton(systemImage: "chevron.down", fg: PollyTheme.text, cancelShortcut: true) { closeModal(dismiss) }
            VStack(spacing: 3) {
                Text("FLASH CARDS")
    ...
    private var generateHero: some View {
        VStack(spacing: 18) {
            ...
            if FlashDeckBuilder.shared.isBuilding(topicId) {
                ...
                Text("Polly is writing your cards…")
                ...
                Text("Build this topic's deck")
                    ...
                    .tracking(-0.4)
                    .foregroundStyle(PollyTheme.text)
                Text("Polly reads everything you've saved here and writes flash cards that test the ideas that matter.")
                    .font(.system(size: 14))
                    .lineSpacing(3)
                    .foregroundStyle(PollyTheme.text2)
                    .multilineTextAlignment(.center)
    ...
    private func countChip(_ n: Int) -> some View {
        Button {
            ...
            Text("\(n)")
                ...
                .foregroundStyle(generateCount == n ? PollyTheme.inkOnAccent : PollyTheme.text2)
                .frame(width: 44, height: 40)
                .background(
                    generateCount == n ? PollyTheme.accent : PollyTheme.surface,
                    in: RoundedRectangle(cornerRadius: 12)
                )
                .overlay(RoundedRectangle(cornerRadius: 12).stroke(PollyTheme.border, lineWidth: generateCount == n ? 0 : 1))
        }
        .buttonStyle(.plain)
    }
    ...
    private var moreCardsLabel: some View {
        ...
        HStack(spacing: 4) {
            if building {
                ProgressView().tint(PollyTheme.text2).scaleEffect(0.7)
            } else {
    ...
    private var modeButtons: some View {
        VStack(spacing: 10) {
            ...
            modeButton("text", icon: "keyboard", title: "Type answers",
                       sub: "Write it in your own words, Polly grades")
            modeButton("voice", icon: "mic.fill", title: "Voice round",
                       sub: "Polly quizzes you out loud, game-show style")
        }
    }
            ...
            ForEach(sets) { s in
                HStack(spacing: 10) {
                    ...
                    Text("\(s.score)/\(s.total)")
                        .font(.system(size: 15, weight: .bold))
                        .foregroundStyle(s.total > 0 && Double(s.score) / Double(s.total) >= 0.9 ? PollyTheme.gold : PollyTheme.text)
    ...
    private func setRating(_ card: FlashCard, _ rating: String?) {
        ...
        Task {
            do {
                _ = try await PollyAPI.shared.rateFlashCard(cardId: card.id, rating: rating)
            } catch {
    ...
    private var addQuestionRow: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text("ADD YOUR OWN")
                ...
                .padding(.top, 8)
            HStack(spacing: 8) {
                TextField("Write a question — Polly fills in the answer…", text: $newQuestion, axis: .vertical)
    ...
    private func load() async {
        ...
        defer { loading = false }
        do {
            let flash = try await PollyAPI.shared.getTopicFlash(id: topicId)
            cards = flash.cards
            sets = flash.sets
            stars = flash.stars
        } catch {
    ...
    private func addCard() {
        ...
        Task {
            do {
                let card = try await PollyAPI.shared.authorFlashCard(
                    topicId: topicId, question: question, model: PollyChatModel.current.rawValue)
                cards.append(card)
                newQuestion = ""
    ...
    private func startSet(mode: String) {
        ...
        Task {
            do {
                let start = try await PollyAPI.shared.startFlashSet(threadId: topicId, mode: mode)
                if mode == "voice" {
                    voiceSet = start
                } else {
                    activeSet = start
...
struct FlashCardEditSheet: View {
        ...
        Task {
            do {
                let distractors = [d1, d2, d3]
                    ...
                    .filter { !$0.isEmpty }
                let updated = try await PollyAPI.shared.updateFlashCard(
                    cardId: card.id,
    ...
    private func delete() {
        ...
        Task {
            do {
                try await PollyAPI.shared.deleteFlashCard(cardId: card.id)
                onDelete()
                closeModal(dismiss)
            } catch {
                errorMessage = error.localizedDescription
