...
struct VoiceSessionView: View {
    ...
    private var tapeText: String {
        (title ?? "Polly voice session").uppercased()
    }
    ...
    private var headerRow: some View {
        HStack {
            ...
            .padding(.trailing, 10)
            .padding(.vertical, 6)
            .background(PollyTheme.surface.opacity(0.7), in: Capsule())
        ...
        case .connected:
            if holdToTalk {
                return client.talking ? "Listening…"
                    : client.status == "Thinking" ? "Polly is thinking…"
                    : "Press and hold the button to talk."
            }
            return "Polly is listening — just talk."
        case .speaking:
            return holdToTalk ? "Polly is speaking — press the button to cut in." : "Polly is speaking…"
        ...
        VStack(spacing: 8) {
            Button(action: action) {
                Image(systemName: systemImage)
                    ...
                    .foregroundStyle(danger ? .white : PollyTheme.text)
                    .frame(width: 58, height: 58)
                    .background(
                        Circle().fill(danger ? Color(hex: 0xC84A3C) : PollyTheme.surface2)
                    )
                    .overlay(
                        Circle().stroke(danger ? Color(hex: 0xE0635A) : PollyTheme.border, lineWidth: 1)
                    )
            ...
            Text(label)
                .font(.system(size: 11, weight: .semibold))
                .tracking(0.2)
                .foregroundStyle(danger ? Color(hex: 0xE88A82) : PollyTheme.text2)
        }
    }
}
