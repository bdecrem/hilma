...
struct TopicContextSheet: View {
    ...
    @State private var sources: [PollyAPI.TopicSource] = []
    @State private var summaries: [PollyAPI.SummaryVersion] = []
    @State private var currentSummaryId: String? = nil
    @State private var readerTarget: PollyAPI.SummaryVersion? = nil
    @State private var sourceReaderTarget: PollyAPI.TopicSource? = nil
    @State private var loading = true
    @State private var loadError: String? = nil
    @State private var deleting: Set<String> = []
    @State private var confirmDelete: PollyAPI.TopicSource? = nil
    ...
    // in load() (the list's PollyTopic can be stale after an earlier edit).
    ...
    var body: some View {
        ...
        .alert("Add context",
            ...
            Text(addError ?? "Paste a URL (article, YouTube, etc.) — Polly pulls it in and adds it to this topic's context.")
        }
        .fileImporter(
    ...
    private var addContextButton: some View {
        Button { addURL = ""; addError = nil; showAddContext = true } label: {
            HStack(spacing: 7) {
                if addBusy {
                    ProgressView().controlSize(.mini).tint(PollyTheme.text2)
                } else {
    ...
    private func importNotes(_ result: Result<[URL], Error>) {
        ...
        Task {
            ...
            do {
                ...
                _ = try await PollyAPI.shared.uploadTopicNotes(
                    id: topicId,
                    text: text,
    ...
    private func commitFocus(_ newValue: String) async {
        ...
        do {
            let cardCount = try await PollyAPI.shared.setStudyFocus(id: topicId, focus: trimmed)
            ...
            if cardCount > 0 {
                ...
                Task {
                    _ = try? await PollyAPI.shared.redoFlashDeck(topicId: topicId, instructions: instructions)
    ...
    private func commitAdd() async {
        ...
        defer { addBusy = false }
        do {
            _ = try await PollyAPI.shared.addTopicSource(id: topicId, url: url)
            addURL = ""
            await load()
        } catch {
    ...
    private func readerHeading(_ src: PollyAPI.TopicSource) -> String {
    ...
    private func isReadable(_ src: PollyAPI.TopicSource) -> Bool {
    ...
    private func summaryRow(_ v: PollyAPI.SummaryVersion) -> some View {
            ...
            VStack(alignment: .leading, spacing: 4) {
                HStack(spacing: 8) {
                    Text(v.instructions == nil ? "BASE SUMMARY" : "REVISED")
                        .font(.system(size: 10, weight: .bold))
                        .tracking(0.5)
                        .foregroundStyle(v.instructions == nil ? PollyTheme.text3 : PollyTheme.accent)
    ...
    private func summaryMeta(_ v: PollyAPI.SummaryVersion) -> String {
        var bits: [String] = []
    ...
    @ViewBuilder
    private func row(_ src: PollyAPI.TopicSource) -> some View {
            ...
            VStack(alignment: .leading, spacing: 4) {
                HStack(spacing: 8) {
                    Text(rowTag(src))
                        ...
                        .tracking(0.5)
                        .foregroundStyle(
                            src.note == true ? PollyTheme.gold
                            : src.kind == "primary" ? PollyTheme.accent : PollyTheme.text3)
                    if src.part != "url" {
    ...
    @ViewBuilder
    private func quoteContent(_ src: PollyAPI.TopicSource) -> some View {
        VStack(alignment: .leading, spacing: 6) {
            Text("QUOTE")
                .font(.system(size: 10, weight: .bold))
                .tracking(0.5)
    ...
    ///   QUOTE
    private func rowTag(_ src: PollyAPI.TopicSource) -> String {
    ...
    private func load() async {
        ...
        do {
            ...
            async let sourcesTask = PollyAPI.shared.listTopicSources(id: topicId)
            async let summariesTask = try? PollyAPI.shared.listSummaries(id: topicId)
            async let threadTask = try? PollyAPI.shared.getThread(id: topicId)
    ...
    private func delete(_ src: PollyAPI.TopicSource) async {
        deleting.insert(src.id)
        defer { deleting.remove(src.id) }
        do {
            try await PollyAPI.shared.deleteTopicSource(
                id: topicId,
                kind: src.kind,
