...
struct FlashSetView: View {
    ...
    @State private var pebbles: [PollyArtifact] = []
    @State private var gradingPebble: PollyArtifact? = nil
    ...
    var body: some View {
        ...
        .task {
            pebbles = (try? await PollyAPI.shared.listArtifacts()) ?? []
    ...
    private var playView: some View {
        VStack(spacing: 0) {
            ...
            if prefilledCount > 0 {
                ...
                .overlay(Capsule().stroke(PollyTheme.borderSoft, lineWidth: 1))
                .padding(.top, 10)
            }
            Spacer(minLength: 8)
    ...
    private var header: some View {
        HStack {
            ...
            Spacer()
            VStack(spacing: 2) {
                Text(start.jumboLevel != nil
                     ? L("PECK · LEVEL \(start.jumboLevel!)", "PECK · LIVELLO \(start.jumboLevel!)", "PECK · NIVEAU \(start.jumboLevel!)", "PECK · 레벨 \(start.jumboLevel!)")
                     : L("FLASH ROUND", "GIRO FLASH", "MANCHE FLASH", "플래시 라운드"))
                    .font(.system(size: 11, weight: .bold))
                    .tracking(1.2)
    ...
    private var progressRow: some View {
        HStack(spacing: 4) {
            ForEach(0..<questions.count, id: \.self) { i in
                Capsule()
                    .fill(i < index ? PollyTheme.accent : (i == index ? PollyTheme.accentDim : PollyTheme.surface2))
                    .frame(height: 5)
            }
        }
    ...
    private func questionCard(_ q: FlashQuestion) -> some View {
        ...
        .overlay(RoundedRectangle(cornerRadius: 20).stroke(PollyTheme.border, lineWidth: 1))
    ...
    private func rate(_ cardId: String, _ rating: String?) {
        ...
        Task {
            ...
            try? await PollyAPI.shared.rateFlashCard(cardId: cardId, rating: rating)
        }
    }
    ...
    private var textAnswerRow: some View {
        VStack(spacing: 10) {
            TextField("Type your answer…", text: $draft, axis: .vertical)
                ...
                .padding(.horizontal, 16)
                .padding(.vertical, 13)
                .background(PollyTheme.bgRaised, in: RoundedRectangle(cornerRadius: 16))
    ...
    private func clozeAnswerRow(_ q: FlashQuestion) -> some View {
        VStack(spacing: 10) {
            TextField("Type the missing word…", text: $draft)
                ...
                .foregroundStyle(clozeVerdict == nil ? PollyTheme.text : (clozeVerdict! ? Color(hex: 0x2E9E63) : Color(hex: 0xE0635A)))
                .tint(PollyTheme.accent)
            ...
            if clozeVerdict == false, let word = q.clozeAnswer {
                (Text("The word: ").foregroundStyle(PollyTheme.text2)
                    + Text(word).bold().foregroundStyle(PollyTheme.text))
                    .font(.system(size: 15))
                    .frame(maxWidth: .infinity, alignment: .leading)
                    .transition(.opacity)
            }
    ...
    private var clozeFill: Color {
        guard let v = clozeVerdict else { return PollyTheme.bgRaised }
        return v ? Color(hex: 0x46D18A).opacity(0.16) : Color(hex: 0xE0635A).opacity(0.14)
    }

    private var clozeStroke: Color {
        guard let v = clozeVerdict else { return PollyTheme.border }
        ...
        Task {
            do {
                let payload = zip(questions, answers).map {
                    PollyAPI.FlashAnswer(card_id: $0.cardId, answer: $1, format: $0.format)
                }
                let result = try await PollyAPI.shared.submitFlashSet(
                    mode: start.mode,
    ...
    private var gradingView: some View {
        VStack(spacing: 16) {
            ...
            Text(start.mode == "text" || start.mode == "mixed" ? "Polly is grading your answers…" : "Tallying your round…")
...
struct FlashResultsView: View {
    ...
    private var scoreRing: some View {
        ZStack {
            Circle()
                .stroke(PollyTheme.surface2, lineWidth: 12)
            Circle()
                .trim(from: 0, to: ringProgress)
                .stroke(
                    isPerfect ? PollyTheme.gold : PollyTheme.accent,
...
struct MissClinicSheet: View {
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 18) {
                ...
                clinicRow(icon: "plus.circle", title: "Add a new card",
                          sub: "Type the question; Polly writes the answer") {
                ...
                if newCardOpen {
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        switch newCardState {
                        case .writing:
                            Label("Polly is writing the card…", systemImage: "hourglass")
                                .font(.system(size: 12.5, weight: .medium))
                                .foregroundStyle(PollyTheme.text2)
                        case .added:
    ...
    private func addNewCard() {
        ...
        Task {
            do {
                _ = try await PollyAPI.shared.authorFlashCard(threadId: threadId, question: q)
                newCardQuestion = ""
                newCardState = .added
            } catch {
                newCardState = .failed
    ...
    private func deleteCard() {
        ...
        Task {
            try? await PollyAPI.shared.deleteFlashCard(cardId: row.cardId)
    ...
    private func setRating(_ newValue: String?) {
        ...
        Task { try? await PollyAPI.shared.rateFlashCard(cardId: row.cardId, rating: newValue) }
    }
    ...
    private func saveNote() {
        ...
        savedNote = trimmed
        noteSaved = true
        Task { try? await PollyAPI.shared.setGradingNote(cardId: row.cardId, note: trimmed) }
    }
    ...
    private func clinicChip<Icon: View>(
        ...
        Button(action: action) {
            ...
            .foregroundStyle(active ? activeTint : PollyTheme.text2)
            .padding(.horizontal, 13)
            .padding(.vertical, 9)
            .background(PollyTheme.surface, in: Capsule())
            .overlay(Capsule().stroke(active ? activeTint.opacity(0.5) : PollyTheme.border, lineWidth: 1))
        }
        .buttonStyle(.plain)
    }
...
struct ConfettiView: View {
    private let colors: [Color] = [
        PollyTheme.accent, PollyTheme.gold, Color(hex: 0x46D18A),
