...
struct StreakCelebrationView: View {
        ...
        ZStack {
            ...
            VStack(spacing: 22) {
                ...
                Text(milestone.celebration
                     ? L("\(milestone.days)-day streak!", "\(milestone.days) giorni di fila!", "\(milestone.days) jours d’affilée\u{202F}!", "\(milestone.days)일 연속!")
                ...
                Button(action: onDismiss) {
                    Text(L("Keep it going", "Continua così", "Continue comme ça", "계속 이어가요"))
    ...
    private func sparkleImage(_ p: Sparkle, in size: CGSize) -> some View {
        ...
        return Image(systemName: p.id % 2 == 0 ? "sparkles" : "sparkle")
            ...
            .foregroundStyle(PollyTheme.gold.opacity(sparkleOn ? 0.85 : 0.25))
