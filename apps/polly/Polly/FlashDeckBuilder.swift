...
final class FlashDeckBuilder {
    ...
    func generate(topicId: String, topicLabel: String, count: Int, model: String?) {
        ...
        Task {
            do {
                let cards = try await PollyAPI.shared.generateFlashCards(
    ...
    func redo(topicId: String, topicLabel: String, instructions: String, model: String?) {
        ...
        Task {
            do {
                let cards = try await PollyAPI.shared.redoFlashDeck(
    ...
    var body: some View {
        HStack(spacing: 9) {
            Image(systemName: toast.isError ? "exclamationmark.triangle.fill" : "bolt.fill")
                ...
                .foregroundStyle(toast.isError ? Color(hex: 0xE0635A) : PollyTheme.gold)
            Text(toast.message)
