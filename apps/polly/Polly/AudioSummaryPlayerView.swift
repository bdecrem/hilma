    ...
    private var speedControl: some View {
        Button { ctl.cycleSpeed() } label: {
            Text(speedLabel)
                ...
                .foregroundStyle(ctl.speed == 1.0 ? PollyTheme.text3 : PollyTheme.accent)
                .frame(minWidth: 40)
                .padding(.vertical, 7)
                .padding(.horizontal, 14)
                .background(PollyTheme.surface, in: Capsule())
                .overlay(Capsule().stroke(ctl.speed == 1.0 ? PollyTheme.border : PollyTheme.accent.opacity(0.5), lineWidth: 1))
        }
        .buttonStyle(.plain)
