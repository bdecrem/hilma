...
struct VoiceSettingsView: View {
            ...
            SettingsSection(label: "How it talks") {
                SettingsCard {
                    ...
                    if styleText.trimmingCharacters(in: .whitespacesAndNewlines) != savedStyle {
                        SettingsDivider()
                        SettingsRow(label: "Save style", labelColor: PollyTheme.accent) {
                            Task { await save() }
                        }
                    }
                }
        ...
        return HStack(spacing: 12) {
            Button {
                ...
                Image(systemName: playingVoice == v.id ? "stop.circle.fill" : "play.circle.fill")
                    .font(.system(size: 26))
                    .foregroundStyle(playingVoice == v.id ? PollyTheme.accent : PollyTheme.slate)
            }
            .buttonStyle(.plain)
    ...
    private func load() async {
        do {
            let p = try await PollyAPI.shared.voicePrefs()
            prefs = p
    ...
    private func save() async {
        ...
        do {
            let p = try await PollyAPI.shared.saveVoicePrefs(voice: voice, style: style)
            prefs = p
            selected = p.voice ?? p.defaultVoice
            savedStyle = p.voiceStyle ?? ""
        } catch {
