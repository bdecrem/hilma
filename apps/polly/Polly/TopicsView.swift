...
struct TopicsView: View {
    ...
    @State private var flashTarget: PollyTopic? = nil
    ...
    @State private var bookReaderTarget: PollyTopic? = nil
    ...
    private func applySort(_ list: [PollyTopic]) -> [PollyTopic] {
        switch sort {
        case .recent:
    ...
    var body: some View {
        ...
        .task {
            if topics.isEmpty, let cached: [PollyTopic] = ScreenCache.load(key: ScreenCache.topics) {
                topics = cached
            }
    ...
    private var newTopicButton: some View {
        Button { showNewTopic = true } label: {
            Image(systemName: "plus")
                ...
                .background(PollyTheme.accent, in: Circle())
        ...
        Button { communityPresented = true } label: {
            HStack(spacing: 12) {
                Image(systemName: "person.2.fill")
                    ...
                    .background(PollyTheme.accent.opacity(0.13), in: Circle())
    ...
    @ViewBuilder
    private func topicList(_ display: [PollyTopic]) -> some View {
        ...
        HStack(spacing: 5) {
            if let systemImage {
                Image(systemName: systemImage)
                    ...
                    .foregroundStyle(tint ?? PollyTheme.text3)
            }
    ...
    private func load() async {
        ...
        defer { loading = false }
        do {
            topics = try await PollyAPI.shared.listTopics()
        ...
        // on a failure.
        Task {
            if let fresh = try? await PollyAPI.shared.jumboState() {
                jumbo = fresh
    ...
    private func generateAudio(_ topic: PollyTopic) {
        Task {
            do {
                let pending = try await PollyAPI.shared.generateAudioSummary(
                    id: topic.id, model: PollyChatModel.current.rawValue)
                if let idx = topics.firstIndex(where: { $0.id == topic.id }) {
                    topics[idx].audioSummary = pending ?? PollyAudioSummary(
    ...
    private func generateBook(_ topic: PollyTopic) {
        Task {
            do {
                let pending = try await PollyAPI.shared.generateBookSummary(id: topic.id)
                if let idx = topics.firstIndex(where: { $0.id == topic.id }) {
                    topics[idx].bookSummary = pending
                        ?? PollyBookSummary(status: "generating", error: nil, updatedAt: nil)
                }
            } catch let PollyAPIError.http(409, _) {
    ...
    private func startAudioPollingIfNeeded() {
        ...
        Task {
            ...
            while anySummaryGenerating {
                try? await Task.sleep(for: .seconds(6))
                do { topics = try await PollyAPI.shared.listTopics() } catch { break }
            }
        }
    }

    private func startRename(_ topic: PollyTopic) {
        renameTarget = topic
    }

    private func commitRename(_ target: PollyTopic, newName: String, newKind: String?) {
        renameTarget = nil
        ...
        Task {
            do {
                try await PollyAPI.shared.renameTopic(
                    id: target.id, to: name, kind: kindChanged ? newKind : nil)
            } catch { await load() }
        }
    }

    private func delete(_ topic: PollyTopic) {
        let prev = topics
        topics.removeAll { $0.id == topic.id }
        Task {
            do { try await PollyAPI.shared.deleteTopic(id: topic.id) }
            catch { topics = prev }
        }
    }
    ...
    private func toggleShare(_ topic: PollyTopic) {
        ...
        Task {
            do { try await PollyAPI.shared.setShared(topicId: topic.id, shared: willShare) }
            catch {
                topics = prev
    ...
    private func togglePin(_ topic: PollyTopic) {
        ...
        Task {
            do { try await PollyAPI.shared.setPinned(id: topic.id, pinned: willPin) }
            catch { topics = prev }
        }
    }
}
...
/// What the Topic Context sheet needs to open — decoupled from PollyTopic so
...
struct NewTopicSheet: View {
    ...
    private func create() {
        ...
        Task {
            do {
                let id = try await PollyAPI.shared.createTopic(title: name, kind: kind)
                onCreated(id, name)
                closeModal(dismiss)
            } catch {
...
struct RenameTopicSheet: View {
    ...
    init(topic: PollyTopic, onSave: @escaping (String, String?) -> Void) {
        self.topic = topic
        self.onSave = onSave
