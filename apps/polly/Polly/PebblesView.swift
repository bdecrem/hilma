...
struct PebblesView: View {
    ...
    @State private var artifacts: [PollyArtifact] = []
    ...
    private func load() async {
        ...
        loadError = nil
        do {
            let all = try await PollyAPI.shared.listArtifacts()
    ...
    private func delete(_ id: String) {
        ...
        page = min(page, max(0, artifacts.count - 1))
        Task { try? await PollyAPI.shared.deleteArtifact(id: id) }
    }
}
...
struct PebbleQuoteCard: View {
    ...
    private var imageCard: some View {
        ...
        .frame(maxWidth: .infinity,
               ...
               alignment: .leading)
        .background(PollyTheme.surface, in: RoundedRectangle(cornerRadius: 24))
        .overlay(RoundedRectangle(cornerRadius: 24).stroke(PollyTheme.border, lineWidth: 1))
...
struct PebbleImage: View {
            ...
            case .success(let img):
                if matted {
                    ...
                    img.resizable()
                        ...
                        .padding(10)
                        .background(PollyTheme.surface2, in: RoundedRectangle(cornerRadius: corner))
                } else {
                    img.resizable()
                        .aspectRatio(contentMode: .fit)
    ...
    private func placeholder(_ icon: Image?) -> some View {
        ZStack {
            PollyTheme.surface2
            if let icon {
                icon.font(.system(size: 26)).foregroundStyle(PollyTheme.text3)
            } else {
                ProgressView().tint(PollyTheme.accent)
            }
        }
        .frame(minHeight: 160)
    }
...
struct PebbleAddSheet: View {
    ...
    @State private var pickedTopic: PollyTopic? = nil
        ...
        VStack(spacing: 0) {
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 16) {
                        ...
                        Menu {
                            ...
                            HStack(spacing: 8) {
                                ...
                                Text(pickedTopic?.topic ?? "Link to a topic (optional)")
                                    ...
                                    .foregroundStyle(pickedTopic == nil ? PollyTheme.text3 : PollyTheme.text)
                                    .lineLimit(1)
                                Spacer()
                    ...
                    Button { save() } label: {
                        HStack(spacing: 8) {
                            if saving { ProgressView().tint(PollyTheme.inkOnAccent) }
    ...
    private func save() {
        ...
        Task {
            do {
                ...
                if let photoData {
                    _ = try await PollyAPI.shared.createImageArtifact(
                        imageData: photoData,
                    ...
                    _ = try await PollyAPI.shared.createArtifact(
                        body: body,
                        source: trimmedSource.isEmpty ? nil : trimmedSource,
                        threadId: pickedTopic?.id
                    )
