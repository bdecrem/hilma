...
struct OnboardingView: View {
    ...
    var body: some View {
        ...
        .alert("Polly", isPresented: Binding(
    ...
    private var pageOne: some View {
        VStack(spacing: 20) {
            ...
            Text("Feed it a book, an article, or a YouTube video. Polly turns it into a topic you can talk with — and helps you actually remember it.")
                .font(.system(size: 16))
                .lineSpacing(4)
        ...
        if mode == .firstRun && page == pageCount - 1 {
            VStack(spacing: 12) {
                Button {
                    ...
                    HStack(spacing: 8) {
                        if startingGuest { ProgressView().tint(PollyTheme.inkOnAccent) }
                        Text(startingGuest ? "Setting up…" : "Try Polly — no account needed")
