...
struct FlashDecksSheet: View {
    ...
    private var totalsStrip: some View {
        ...
        return HStack(spacing: 14) {
            statBit("\(decks.count)", decks.count == 1 ? "DECK" : "DECKS", PollyTheme.text)
            statBit("\(cards)", "CARDS", PollyTheme.text)
            if priority > 0 { statBit("\(priority)", "PRIORITY", PollyTheme.gold) }
            if buried > 0 { statBit("\(buried)", "BURIED", PollyTheme.text3) }
            Spacer()
        }
        ...
        HStack(alignment: .center, spacing: 12) {
            ...
            HStack(spacing: 2) {
                ForEach(0..<3, id: \.self) { i in
                    Image(systemName: i < deck.stars ? "star.fill" : "star")
                        .font(.system(size: 10))
                        .foregroundStyle(i < deck.stars ? PollyTheme.gold : PollyTheme.text3.opacity(0.5))
                }
            }
            ...
            Button {
                ...
                Image(systemName: (deck.peckExcluded ?? false) ? "bolt.slash" : "bolt.fill")
                    ...
                    .foregroundStyle((deck.peckExcluded ?? false) ? PollyTheme.text3 : PollyTheme.gold)
    ...
    private func togglePeck(_ deck: FlashDeck) {
        ...
        Task {
            try? await PollyAPI.shared.setPeckExcluded(id: deck.threadId, excluded: newValue)
        }
    }
    ...
    private func load() async {
        ...
        loadError = nil
        do {
            decks = try await PollyAPI.shared.listFlashDecks()
        } catch {
