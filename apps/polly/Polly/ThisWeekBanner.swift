...
struct ThisWeekBanner: View {
    ...
    private func card(_ hit: (topic: PollyTopic, due: Date)) -> some View {
        HStack(spacing: 11) {
            ...
            (Text(L("This week: ", "Questa settimana: ", "Cette semaine : ", "이번 주: "))
             ...
             + Text(L(" refresher — due \(dayWord(hit.due)).",
                      " — ripasso da fare \(dayWord(hit.due)).",
                      " — révision à faire \(dayWord(hit.due)).",
                      " 복습 — \(dayWord(hit.due))까지.")).foregroundColor(PollyTheme.text2))
        ...
        .padding(.vertical, 10)
        .background(PollyTheme.surface, in: RoundedRectangle(cornerRadius: 13))
        .overlay(RoundedRectangle(cornerRadius: 13).stroke(PollyTheme.gold.opacity(0.35), lineWidth: 1))
    ...
    private var dueThisWeek: (topic: PollyTopic, due: Date)? {
        guard let week = Calendar.current.dateInterval(of: .weekOfYear, for: Date()) else { return nil }
        let now = Date()
        return topics
            .compactMap { t -> (topic: PollyTopic, due: Date)? in
