...
final class GameScene: SKScene {
    ...
    init(size: CGSize, track: TrackDef, chart: [ChartNote], skin: Skin, config: RunConfig,
         settings: RunSettings, haptics: Haptics, onFinish: @escaping (RunResult) -> Void) {
        self.track = track
        self.chart = chart
        self.skin = skin
        self.config = config
    ...
    private func showPausedOverlay() {
        ...
        label.attributedText = kerned(skin.styled("PAUSED — TAP TO RESUME"),
                                      font: skin.laneStyle == .glyphs ? Fonts.monoBold : Fonts.unbounded,
                                      size: 14, color: skin.foreground,
                                      kern: skin.laneStyle == .glyphs ? 4 : 1)
